...
SELECT DISTINCT A.PERSON_UID, A.ID, A.STUDENT_LEVEL
	...
	CASE
		...
		WHEN A.STUDENT_LEVEL IN ('EC','MA')
			THEN ( CASE WHEN SUBSTR(J.STVTERM_DESC,1,6) = '2020-2' THEN 1 ELSE 2 END )
	...
	, CASE
		WHEN (A.STUDENT_LEVEL = 'UG' AND SUBSTR(G.STVTERM_DESC,1,6) = SUBSTR(J.STVTERM_DESC,1,6))
				OR (A.STUDENT_LEVEL IN ('EC','MA') AND SUBSTR(J.STVTERM_DESC,1,6) = '2020-2')
		...
		LEFT JOIN PROGRAMA_ANTERIOR L ON A.PERSON_UID = L.SOVLCUR_PIDM AND A.STUDENT_LEVEL = L.SOVLCUR_LEVL_CODE AND A.PROGRAM = L.PROGRAMA_DESTINO
											AND L.SOVLCUR_SEQNO = (SELECT MAX(L1.SOVLCUR_SEQNO) FROM PROGRAMA_ANTERIOR L1
																	WHERE L1.SOVLCUR_PIDM = L.SOVLCUR_PIDM AND L1.SOVLCUR_LEVL_CODE = L.SOVLCUR_LEVL_CODE
																		AND ((SUBSTR(L1.PROGRAMA_DESTINO,4,3) = '-UG' AND L1.SOVLCUR_TERM_CODE <= '220435')
																				OR (SUBSTR(L1.PROGRAMA_DESTINO,4,3) = '-WA' AND L1.SOVLCUR_TERM_CODE <= '220534'))
...
WHERE ((A.ENROLLMENT_STATUS IN ('EL','RF','RO','SE') AND B.PERSON_UID IS NOT NULL) OR A.ENROLLMENT_STATUS IN ('RM','TP','NI','NS')) --AND A.PERSON_UID IN (5216,5171,188158,217951)
	AND ((A.STUDENT_LEVEL ='UG' AND A.ACADEMIC_PERIOD IN ('220435','220534'))
		OR (A.STUDENT_LEVEL = 'EC' AND A.ACADEMIC_PERIOD = '220954' AND A.PROGRAM = 'MOINTL')
		OR (A.STUDENT_LEVEL = 'MA' AND A.ACADEMIC_PERIOD IN (/*'219839',*/'220813','220843') AND (A.ACADEMIC_PERIOD = A.ACADEMIC_PERIOD_ADMITTED OR A.ACADEMIC_PERIOD_ADMITTED IS NULL)))
;
