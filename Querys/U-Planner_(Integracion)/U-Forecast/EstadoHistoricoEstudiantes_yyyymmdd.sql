...
SELECT DISTINCT SFRSTCR_PIDM AS "id_estudiante"
	...
	, COALESCE(CASE 
				...
				WHEN H.HOLD IN ('BA','SD') THEN 'RD'
				END
			, MAX(CASE WHEN SFRSTCR_RSTS_CODE IN ('RW','RE','RA') THEN 'C' END) OVER(PARTITION BY SFRSTCR_PIDM,SFRSTCR_TERM_CODE)
			, MAX(CASE WHEN SFRSTCR_RSTS_CODE IN ('WC','RF') THEN 'RT' END) OVER(PARTITION BY SFRSTCR_PIDM,SFRSTCR_TERM_CODE)
			, 'D') AS "id_estado"
FROM ODSMGR.SFRSTCR
