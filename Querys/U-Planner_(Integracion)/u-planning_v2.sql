...
FROM ODSMGR.LOE_PROGRAM_AREA_PRIORITY, ODSMGR.LOE_SMRPRLE
...
FROM ODSMGR.LOE_SIBINST S, ODSMGR.LOE_SPRIDEN
...
FROM ODSMGR.LOE_PROGRAM_AREA_PRIORITY A, ODSMGR.LOE_AREA_COURSE B, ODSMGR.LOE_SMRARUL C, ODSMGR.COURSE_CATALOG D
WHERE A.TERM_CODE_EFF = B.TERM_CODE_EFF AND A.AREA = B.AREA_COURSE
    AND B.TERM_CODE_EFF = C.SMRARUL_TERM_CODE_EFF(+) AND B.AREA_COURSE = C.SMRARUL_AREA(+) AND B.AREA_RULE = C.SMRARUL_KEY_RULE(+)
...
FROM ODSMGR.LOE_SOBCURR, ODSMGR.LOE_PROGRAM_AREA_PRIORITY, ODSMGR.LOE_STVTERM
WHERE SOBCURR_PROGRAM = PROGRAM
    ...
    AND STVTERM_CODE IN ('221402');    --INGRESAR_PERIODO_NUEVO
...
FROM ODSMGR.SLBRDEF, ODSMGR.SLBBLDG, ODSMGR.STVBLDG
...
FROM ODSMGR.SLBRDEF, ODSMGR.SLBBLDG, ODSMGR.LOE_SLRRDEF, ODSMGR.STVRDEF
WHERE SLBRDEF_BLDG_CODE = SLBBLDG_BLDG_CODE
...
SELECT DISTINCT 'UPN' AS "id_institucion"
    ...
    , NULL AS "modalidad"
    , NULL AS "sesion"
FROM ODSMGR.LOE_PROGRAM_AREA_PRIORITY A, ODSMGR.LOE_AREA_COURSE B, ODSMGR.LOE_SMRARUL C, ODSMGR.COURSE_CATALOG D, ODSMGR.STVCAMP, ODSMGR.STVTERM
WHERE A.TERM_CODE_EFF = B.TERM_CODE_EFF AND A.AREA = B.AREA_COURSE
    AND B.TERM_CODE_EFF = C.SMRARUL_TERM_CODE_EFF(+) AND B.AREA_COURSE = C.SMRARUL_AREA(+) AND B.AREA_RULE = C.SMRARUL_KEY_RULE(+)
    --AND A.TERM_CODE_EFF = D.ACADEMIC_PERIOD
    AND CASE WHEN B.AREA_RULE IS NULL THEN B.SUBJ_CODE ELSE C.SMRARUL_SUBJ_CODE END = D.SUBJECT
...
SELECT /* DISTINCT*/'UPN' AS "id_institucion"
    ...
    , NULL AS "modalidad"
    , NULL AS "sesion"
FROM ODSMGR.LOE_PROGRAM_AREA_PRIORITY A, ODSMGR.LOE_AREA_COURSE B, ODSMGR.LOE_SMRARUL C, ODSMGR.LOE_SSBSECT
...
SELECT DISTINCT A.CAMPUS AS "id_campus"
    ...
    , NULL AS "sabado"
    , NULL AS "domingo"
FROM ODSMGR.SCHEDULE_OFFERING A, ODSMGR.MEETING_TIME B, ODSMGR.INSTRUCTIONAL_ASSIGNMENT C, ODSMGR.STVMEET D
WHERE A.COURSE_REFERENCE_NUMBER = B.COURSE_REFERENCE_NUMBER (+)
    AND A.ACADEMIC_PERIOD = B.ACADEMIC_PERIOD(+)
    AND B.COURSE_REFERENCE_NUMBER = C.COURSE_REFERENCE_NUMBER (+)
    AND B.ACADEMIC_PERIOD = C.ACADEMIC_PERIOD(+)
...
SELECT DISTINCT A.CAMPUS AS "id_campus"
    ...
    , 'HT' AS "id_actividad"
FROM ODSMGR.SCHEDULE_OFFERING A, ODSMGR.MEETING_TIME B, ODSMGR.INSTRUCTIONAL_ASSIGNMENT C
WHERE A.COURSE_REFERENCE_NUMBER = B.COURSE_REFERENCE_NUMBER (+)
    AND A.ACADEMIC_PERIOD = B.ACADEMIC_PERIOD(+)
    AND B.COURSE_REFERENCE_NUMBER = C.COURSE_REFERENCE_NUMBER (+)
    AND B.ACADEMIC_PERIOD = C.ACADEMIC_PERIOD(+)
