...
FROM ODSMGR.LOE_SSBSECT
		INNER JOIN ODSMGR.SSRMEET ON SSBSECT_TERM_CODE = SSRMEET_TERM_CODE AND SSBSECT_CRN = SSRMEET_CRN
		INNER JOIN ODSMGR.COURSE_CATALOG ON SSBSECT_SUBJ_CODE = SUBJECT AND SSBSECT_CRSE_NUMB = COURSE_NUMBER AND SSBSECT_TERM_CODE = ACADEMIC_PERIOD
											AND LECTURE_MIN IS NOT NULL AND LECTURE_MIN > 0 AND STATUS = 'A' AND COLLEGE <> 'GR'
WHERE SSBSECT_SSTS_CODE = 'A' AND SSBSECT_MAX_ENRL > 0 AND SSBSECT_ENRL > 0 AND SSBSECT_SUBJ_CODE NOT IN ('ACAD','REPS','TEST','XPEN','XSER')
    AND SSBSECT_TERM_CODE IN ('221413','221513')
UNION
SELECT SSBSECT_CAMP_CODE AS "id_campus"
    , SSBSECT_TERM_CODE AS "id_periodo_academico"
    , 'JORNADA UNICA' AS "id_jornada"
...
FROM ODSMGR.SIRASGN
		INNER JOIN ODSMGR.LOE_SIBINST S ON SIRASGN_PIDM = SIBINST_PIDM AND SIBINST_FCST_CODE = 'AC' AND SIBINST_SCHD_IND = 'Y'
											AND S.SIBINST_TERM_CODE_EFF = (SELECT MAX(S1.SIBINST_TERM_CODE_EFF) FROM ODSMGR.LOE_SIBINST S1
																			WHERE S1.SIBINST_PIDM = SIRASGN_PIDM AND S1.SIBINST_TERM_CODE_EFF <= SIRASGN_TERM_CODE)
		INNER JOIN ODSMGR.LOE_SPRIDEN ON SIRASGN_PIDM = SPRIDEN_PIDM AND SPRIDEN_CHANGE_IND IS NULL AND SPRIDEN_LAST_NAME NOT LIKE '%INACTIV%'
WHERE SIRASGN_TERM_CODE IN ('221413','221513');
