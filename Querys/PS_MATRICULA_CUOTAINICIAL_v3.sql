...
FROM ( SELECT C.BUSINESS_UNIT, C.INVOICE, C.BILL_TO_CUST_ID, C.BILL_STATUS, C.BILL_TYPE_ID, C.INVOICE_AMOUNT, C.INVOICE_DT
			, C.PO_REF, C.ADD_DTTM, C.CARGO, C.CARGO_MONTO, C.CARGO_MONTO_BRUTO, D.ITEM_STATUS, D.BAL_AMT, D.LAST_ACTIVITY_DT, D.ACCOUNTING_DT
		...
		FROM ( (SELECT A.BUSINESS_UNIT, A.INVOICE, A.BILL_TO_CUST_ID, A.BILL_STATUS, A.BILL_TYPE_ID, A.INVOICE_AMOUNT
					...
					, SUM(CASE WHEN 
						(CASE 
	                        ...
	                        WHEN UPPER(B.DESCR) LIKE '%MATR%' OR UPPER(B.DESCR) LIKE '%PRONABEC%MAT%' THEN 'MATRICULA' 
	                        --WHEN UPPER(B.DESCR) LIKE '%2DA%' OR UPPER(B.DESCR) LIKE '%3RA%' THEN 'T2/T3' 
	                        ELSE NULL END) IS NOT NULL THEN B.NET_EXTENDED_AMT ELSE 0 END) AS CARGO_MONTO_BRUTO
				FROM (ODSMGR.PS_BI_HDR A
				...
				WHERE A.BUSINESS_UNIT = 'PER03' AND A.BILL_STATUS IN ('NEW','INV') AND A.BILL_TYPE_ID IN ('B1','F1')
                	AND A.PO_REF IN ('221434','221534') --INGRESAR_PERIODO
                	AND A.BILL_TO_CUST_ID = 'N00307220'
...
WHERE E.ADD_DTTM = E.MAX_ADD_DTTM AND E.BILL_STATUS = 'INV' AND (E.CARGO_MONTO <> 0 OR E.CARGO_MONTO_BRUTO <> 0)
