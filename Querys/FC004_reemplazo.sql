--FC004_reemplazo_
SELECT I.PO_REF AS PERIODO, I.BILL_TO_CUST_ID AS ID_ESTUDIANTE, I.BILL_TYPE_ID AS TIPO_DOCUMENTO, I.ITEM AS INVOICE, I.INVOICE2 AS NRO_GOBIERNO, I.INVOICE_AMOUNT AS MONTO_TRANSACCION
	, J.LINE_SEQ_NUM AS SEQ, J.DESCR AS DESCRIPCION, J.NET_EXTENDED_AMT AS MONTO_DESCRIPCION, I.INSTALL_NBR AS NRO_CUOTA, I.INVOICE_DT AS FECHA_EMISION, N.DUE_DT AS FECHA_VENCIMIENTO
	, I.ENTRY_AMT AS SALDO, I.INVOICE_AMOUNT-I.ENTRY_AMT AS MONTO_PAGADO, CASE WHEN I.ENTRY_AMT = 0 THEN 'TOTAL' ELSE 'PARCIAL' END AS TIPO_PAGO, MAX(O.PAYMENT_METHOD_CDR) AS FORMA_PAGO
...
FROM ( SELECT B.BUSINESS_UNIT, C.PO_REF, C.BILL_TO_CUST_ID, C.BILL_TYPE_ID, B.ITEM, H.INVOICE2, C.INVOICE_AMOUNT, NVL(D.INSTALL_NBR,G.INSTALL_NBR) AS INSTALL_NBR, C.INVOICE_DT
		...
		FROM ( SELECT A.BUSINESS_UNIT, A.ITEM, A.ITEM_SEQ_NUM, A.ENTRY_AMT, A.ENTRY_TYPE, A.ACCOUNTING_DT, A.DEPOSIT_ID, A.PAYMENT_ID, A.PAYMENT_SEQ_NUM
				...
				WHERE A.BUSINESS_UNIT = 'PER03' AND (A.CUST_ID <> '9999999999' OR A.CUST_ID IS NULL) AND A.ACCOUNTING_DT <= TO_DATE(:FECHA_FIN,'YYYY-MM-DD')
...
GROUP BY I.PO_REF, I.BILL_TO_CUST_ID, I.BILL_TYPE_ID, I.ITEM, I.INVOICE2, I.INVOICE_AMOUNT, J.LINE_SEQ_NUM, J.DESCR, J.NET_EXTENDED_AMT, I.INSTALL_NBR, I.INVOICE_DT, N.DUE_DT
	, I.ENTRY_AMT, I.INVOICE_AMOUNT-I.ENTRY_AMT, CASE WHEN I.ENTRY_AMT = 0 THEN 'TOTAL' ELSE 'PARCIAL' END, I.ENTRY_TYPE, I.ACCOUNTING_DT, L.VALUE2, M.VALUE2
;
