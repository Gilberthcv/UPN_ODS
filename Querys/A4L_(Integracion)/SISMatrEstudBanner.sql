...
SELECT A.SFRSTCR_TERM_CODE AS "SISPeriodo"
	, SFRSTCR_CAMP_CODE AS "SISCampusSeccion"
	, B.SSBSECT_SUBJ_CODE ||'.'|| B.SSBSECT_CRSE_NUMB ||'.'|| A.SFRSTCR_TERM_CODE ||'.'|| A.SFRSTCR_CRN
			||'.'|| CASE WHEN B.SSBSECT_SCHD_CODE = 'VIR' OR B.ssbsect_insm_code = 'V' THEN 'V' ELSE 'P' END AS "SISSeccion"
	...
	, C.SPRIDEN_ID AS "SISCodEstudiante"
	, COALESCE(F.COURSE_COUNT,0) +1 AS "SISVezMatCurso"
	, TO_CHAR(A.SFRSTCR_ADD_DATE,'YYYY-MM-DD HH24:MI:SS') AS "SISFechaMatricula"
	, G.SOVLCUR_CAMP_CODE AS "SISCampusEstudiante"
	, G.SOVLCUR_PROGRAM AS "SISCodProgEstudiante"
	, H.SMRPRLE_PROGRAM_DESC AS "SISProgrEstudiante"
	, MAX(I.SPRTELE_PHONE_NUMBER) AS "SISMovilEstudiante"
	, MAX(J.GOREMAL_EMAIL_ADDRESS) AS "SISEmailPersEstudiante"
