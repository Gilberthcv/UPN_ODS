...
SELECT DISTINCT A.PERSON_UID, A.ID, A.STUDENT_LEVEL, CASE WHEN K.MAESTRIA IS NOT NULL THEN A.PROGRAM ||' - '|| K.MAESTRIA ELSE A.PROGRAM END AS PROGRAMA
	...
	, CASE WHEN SUBSTR(G.STVTERM_DESC,1,6) = SUBSTR(H.STVTERM_DESC,1,6) THEN 1 END AS TIPO_PROCESO
	, CASE WHEN A.STUDENT_LEVEL IN ('EC','MA') THEN SUBSTR(H.STVTERM_DESC,1,4) ELSE SUBSTR(H.STVTERM_DESC,1,6) END AS PROCESO_ADMISION_PERIODO_INGRESO
	, CASE
...
FROM ODSMGR.ACADEMIC_STUDY A
		...
		LEFT JOIN ODSMGR.ADMISSIONS_APPLICATION I ON F.SOVLCUR_PIDM = I.PERSON_UID AND F.SOVLCUR_LEVL_CODE = I.STUDENT_LEVEL
													...
													AND I.APPLICATION_NUMBER = (SELECT MAX(I1.APPLICATION_NUMBER) FROM ODSMGR.ADMISSIONS_APPLICATION I1
