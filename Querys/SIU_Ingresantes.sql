...
SELECT DISTINCT A.PERSON_UID, A.ID, A.STUDENT_LEVEL, CASE WHEN L.MAESTRIA IS NOT NULL THEN A.PROGRAM ||' - '|| L.MAESTRIA ELSE A.PROGRAM END AS PROGRAMA
	, CASE
		WHEN K.DNI IS NOT NULL THEN 'DNI - ' || K.DNI
		WHEN K.PASS IS NOT NULL THEN 'PASS - ' || K.PASS
		WHEN K.CNEX IS NOT NULL THEN 'CNEX - ' || K.CNEX
	...
	CASE
		WHEN A.STUDENT_LEVEL = 'UG'
			THEN ( CASE
					...
					WHEN SUBSTR(H.STVTERM_DESC,1,6) = SUBSTR(I.STVTERM_DESC,1,6) THEN 1
					ELSE 2 END )
		WHEN A.STUDENT_LEVEL IN ('EC','MA')
			THEN ( CASE WHEN SUBSTR(I.STVTERM_DESC,1,6) = '2020-1' THEN 1 ELSE 2 END )
	...
	, CASE
		WHEN (A.STUDENT_LEVEL = 'UG' AND SUBSTR(H.STVTERM_DESC,1,6) = SUBSTR(I.STVTERM_DESC,1,6))
				OR (A.STUDENT_LEVEL IN ('EC','MA') AND SUBSTR(I.STVTERM_DESC,1,6) = '2020-1')
		THEN 1 END AS TIPO_PROCESO
	, SUBSTR(I.STVTERM_DESC,1,6) AS PROCESO_ADMISION_PERIODO_INGRESO
	, CASE
	...
	, CASE WHEN K.DNI IS NOT NULL AND LENGTH(K.DNI) < 9 THEN '9233' END AS PAIS_NACIMIENTO
	, CASE WHEN K.DNI IS NOT NULL AND LENGTH(K.DNI) < 9 THEN '85' END AS NACIONALIDAD
	, CASE WHEN K.DNI IS NOT NULL AND LENGTH(K.DNI) < 9 THEN '140101' END AS UBIGEO_NACIMIENTO
	...
	, TO_CHAR(J.APPLICATION_DATE,'DD/MM/YYYY') AS FECHA_INGRESO
...
FROM ODSMGR.ACADEMIC_STUDY A
		...
		LEFT JOIN ODSMGR.LOE_SOVLCUR G ON A.PERSON_UID = G.SOVLCUR_PIDM AND A.STUDENT_LEVEL = G.SOVLCUR_LEVL_CODE
											AND G.SOVLCUR_TERM_CODE_ADMIT = (SELECT MIN(G1.SOVLCUR_TERM_CODE_ADMIT) FROM ODSMGR.LOE_SOVLCUR G1
																			WHERE G1.SOVLCUR_PIDM = G.SOVLCUR_PIDM AND G1.SOVLCUR_LEVL_CODE = G.SOVLCUR_LEVL_CODE AND G1.SOVLCUR_TERM_CODE_ADMIT IS NOT NULL
																				AND (SUBSTR(G1.SOVLCUR_TERM_CODE_ADMIT,4,1) <> '3' OR G1.SOVLCUR_TERM_CODE_ADMIT < '217400')
																				AND G1.SOVLCUR_LMOD_CODE = 'LEARNER' AND G1.SOVLCUR_CACT_CODE = 'ACTIVE')
		LEFT JOIN ODSMGR.STVTERM H ON A.ACADEMIC_PERIOD = H.STVTERM_CODE
		LEFT JOIN ODSMGR.STVTERM I ON --CASE WHEN A.STUDENT_LEVEL IN ('EC','MA') THEN A.ACADEMIC_PERIOD_ADMITTED ELSE G.SOVLCUR_TERM_CODE_ADMIT END = I.STVTERM_CODE
										--CASE WHEN A.STUDENT_LEVEL = 'UG' AND C.COHORT IN ('REINGRESO','NEW_REING') THEN G.SOVLCUR_TERM_CODE_ADMIT ELSE A.ACADEMIC_PERIOD_ADMITTED END = I.STVTERM_CODE
										CASE WHEN A.STUDENT_LEVEL = 'UG' AND A.STUDENT_POPULATION <> 'N' AND F.PERSON_UID IS NULL THEN G.SOVLCUR_TERM_CODE_ADMIT ELSE A.ACADEMIC_PERIOD_ADMITTED END = I.STVTERM_CODE
		LEFT JOIN ODSMGR.ADMISSIONS_APPLICATION J ON G.SOVLCUR_PIDM = J.PERSON_UID AND G.SOVLCUR_LEVL_CODE = J.STUDENT_LEVEL
													--AND CASE WHEN A.STUDENT_LEVEL IN ('EC','MA') THEN A.ACADEMIC_PERIOD_ADMITTED ELSE G.SOVLCUR_TERM_CODE_ADMIT END = J.ACADEMIC_PERIOD
													--AND CASE WHEN A.STUDENT_LEVEL = 'UG' AND C.COHORT IN ('REINGRESO','NEW_REING') THEN G.SOVLCUR_TERM_CODE_ADMIT ELSE A.ACADEMIC_PERIOD_ADMITTED END = J.ACADEMIC_PERIOD
													AND CASE WHEN A.STUDENT_LEVEL = 'UG' AND A.STUDENT_POPULATION <> 'N' AND F.PERSON_UID IS NULL THEN G.SOVLCUR_TERM_CODE_ADMIT ELSE A.ACADEMIC_PERIOD_ADMITTED END = J.ACADEMIC_PERIOD
													AND J.APPLICATION_NUMBER = (SELECT MIN(J1.APPLICATION_NUMBER) FROM ODSMGR.ADMISSIONS_APPLICATION J1
																				WHERE J1.PERSON_UID = J.PERSON_UID AND J1.STUDENT_LEVEL = J.STUDENT_LEVEL
																					AND J1.ACADEMIC_PERIOD = J.ACADEMIC_PERIOD)
		LEFT JOIN DOCUMENTO_IDENTIDAD K ON SPRIDEN_PIDM = K.ENTITY_UID
