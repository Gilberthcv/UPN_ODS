...
SELECT DISTINCT A.PERSON_UID, A.ID, A.STUDENT_LEVEL, CASE WHEN J.MAESTRIA IS NOT NULL THEN A.PROGRAM ||' - '|| J.MAESTRIA ELSE A.PROGRAM END AS PROGRAMA
	, CASE
		WHEN I.DNI IS NOT NULL THEN 'DNI - ' || I.DNI
		WHEN I.PASS IS NOT NULL THEN 'PASS - ' || I.PASS
		WHEN I.CNEX IS NOT NULL THEN 'CNEX - ' || I.CNEX
	...
	CASE
		WHEN C.COHORT IN ('REINGRESO','NEW_REING') THEN 2
	...
	, CASE
		WHEN I.DNI IS NOT NULL AND LENGTH(I.DNI) < 9 THEN 1
		WHEN I.PASS IS NOT NULL AND LENGTH(I.PASS) < 19 THEN 2
		WHEN I.CNEX IS NOT NULL AND LENGTH(I.CNEX) < 19 THEN 3
		ELSE NULL END AS TIPO_DOCUMENTO
	, CASE
		WHEN I.DNI IS NOT NULL AND LENGTH(I.DNI) < 9 THEN I.DNI
		WHEN I.PASS IS NOT NULL AND LENGTH(I.PASS) < 19 THEN I.PASS
		WHEN I.CNEX IS NOT NULL AND LENGTH(I.CNEX) < 19 THEN I.CNEX
	...
	, CASE WHEN I.DNI IS NOT NULL AND LENGTH(I.DNI) < 9 THEN '9233' END AS PAIS_NACIMIENTO
	, CASE WHEN I.DNI IS NOT NULL AND LENGTH(I.DNI) < 9 THEN '85' END AS NACIONALIDAD
	, CASE WHEN I.DNI IS NOT NULL AND LENGTH(I.DNI) < 9 THEN '140101' END AS UBIGEO_NACIMIENTO
	...
	, CASE (CASE WHEN A.PROGRAM = 'MOINTL' THEN J.MAESTRIA ELSE A.PROGRAM END)
		WHEN 'ADM-EA' THEN 1
		WHEN 'ADM-FS' THEN 1
		WHEN 'ADM-UG' THEN 1
		WHEN 'ADM-WA' THEN 1
	...
	, TO_CHAR(H.APPLICATION_DATE,'DD/MM/YYYY') AS FECHA_INGRESO
		...
		LEFT JOIN ODSMGR.LOE_SOVLCUR F ON A.PERSON_UID = F.SOVLCUR_PIDM AND A.STUDENT_LEVEL = F.SOVLCUR_LEVL_CODE --AND F.SOVLCUR_TERM_CODE_ADMIT IS NOT NULL
											AND F.SOVLCUR_TERM_CODE_ADMIT = (SELECT MIN(F1.SOVLCUR_TERM_CODE_ADMIT) FROM ODSMGR.LOE_SOVLCUR F1
																			WHERE F1.SOVLCUR_PIDM = F.SOVLCUR_PIDM AND F1.SOVLCUR_LEVL_CODE = F.SOVLCUR_LEVL_CODE
																				AND F1.SOVLCUR_TERM_CODE_ADMIT IS NOT NULL AND F.SOVLCUR_LMOD_CODE = 'LEARNER' AND F.SOVLCUR_CACT_CODE = 'ACTIVE')
		LEFT JOIN ODSMGR.STVTERM G ON F.SOVLCUR_TERM_CODE_ADMIT = G.STVTERM_CODE
		LEFT JOIN ODSMGR.ADMISSIONS_APPLICATION H ON F.SOVLCUR_PIDM = H.PERSON_UID AND F.SOVLCUR_LEVL_CODE = H.STUDENT_LEVEL AND F.SOVLCUR_TERM_CODE_ADMIT = H.ACADEMIC_PERIOD
													AND H.APPLICATION_NUMBER = (SELECT MAX(H1.APPLICATION_NUMBER) FROM ODSMGR.ADMISSIONS_APPLICATION H1
																				WHERE H1.PERSON_UID = H.PERSON_UID AND H1.STUDENT_LEVEL = H.STUDENT_LEVEL
																					AND H1.ACADEMIC_PERIOD = H.ACADEMIC_PERIOD)
		LEFT JOIN DOCUMENTO_IDENTIDAD I ON SPRIDEN_PIDM = I.ENTITY_UID
		LEFT JOIN MAESTRIA_MOINTL J ON A.PERSON_UID = J.SOVLCUR_PIDM AND A.STUDENT_LEVEL = J.SOVLCUR_LEVL_CODE
...
WHERE A.ENROLLMENT_STATUS IN ('EL','RF','RO','SE')
	AND ((A.STUDENT_LEVEL ='UG' AND A.ACADEMIC_PERIOD IN ('220413','220513'))
		...
		OR (A.STUDENT_LEVEL = 'MA' AND A.ACADEMIC_PERIOD IN ('218816','219813','219839','220813') AND (A.ACADEMIC_PERIOD = F.SOVLCUR_TERM_CODE_ADMIT OR F.SOVLCUR_TERM_CODE_ADMIT IS NULL)))
;
