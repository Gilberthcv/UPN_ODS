...
FROM ODSMGR.LOE_PROGRAM_AREA_PRIORITY A
		...
		*/
		LEFT JOIN ODSMGR.LOE_PROGRAM_AREA_PRIORITY V ON A.PROGRAM = V.PROGRAM AND A.TERM_CODE_EFF = V.TERM_CODE_EFF
														AND V.TERM_CODE_EFF = (SELECT MAX(V1.TERM_CODE_EFF) FROM ODSMGR.LOE_PROGRAM_AREA_PRIORITY V1, ODSMGR.LOE_AREA_COURSE B0
																				WHERE V1.AREA = B0.AREA_COURSE AND V1.TERM_CODE_EFF = B0.TERM_CODE_EFF
																					AND ((SUBSTR(V1.PROGRAM,1,3) IN ('DEH','DER') AND SUBSTR(V1.AREA,LENGTH(V1.AREA)-1) = '12')
																							OR SUBSTR(V1.AREA,LENGTH(V1.AREA)-1) = '10')
																					AND COALESCE(B0.AREA_RULE,B0.SUBJ_CODE) IS NOT NULL AND V1.PROGRAM = V.PROGRAM)
...
--ORDER BY A.PROGRAM, A.TERM_CODE_EFF, SUBSTR(A.AREA,LENGTH(A.AREA)-1)
;