with data as (SELECT SSBSECT_TERM_CODE AS PERIODO, SSBSECT_CAMP_CODE AS CAMPUS, SSRATTR_ATTR_CODE AS COD_PROG, STVATTR_DESC AS PROGRAMA, SSBSECT_SUBJ_CODE AS MATERIA
    , SSBSECT_CRSE_NUMB AS CURSO, SCBCRSE_TITLE AS NOMBRE_CURSO, CICLO, SSBSECT_CRN AS NRC, STVSSTS_DESC AS ESTATUS, SSRXLST_XLST_GROUP AS LST_CRZ
    , SSBSECT_SCHD_CODE AS TIP_HOR, SSBSECT_INSM_CODE AS MET_EDU, SSBSECT_MAX_ENRL AS MAXIMO, SSBSECT_ENRL AS REAL, SSBSECT_SEATS_AVAIL AS RESTANTE
    , SCBCRSE_BILL_HR_LOW AS HRS_CREDITO, SPRIDEN_ID AS ID_DOCENTE, DOCENTE_NAME AS DOCENTE, STVWKLD_DESC AS TIPO_JORNADA, STVFCST_DESC AS ESTADO_DOCENTE
    , SSRMEET_START_DATE AS FECHA_INICIO, SSRMEET_END_DATE AS FECHA_FIN, SSRMEET_MON_DAY AS LUNES, SSRMEET_TUE_DAY AS MARTES, SSRMEET_WED_DAY AS MIERCOLES
    , SSRMEET_THU_DAY AS JUEVES, SSRMEET_FRI_DAY AS VIERNES, SSRMEET_SAT_DAY AS SABADO, SSRMEET_SUN_DAY AS DOMINGO
    , CASE WHEN SSRMEET_BEGIN_TIME IS NOT NULL THEN SSRMEET_BEGIN_TIME ||' - '|| SSRMEET_END_TIME END AS HORA
    , SSRMEET_BLDG_CODE AS EDIFICIO, SSRMEET_ROOM_CODE AS AULA, SSRRPRG_PROGRAM_IND AS INCLUIR_EXCLUIR_PROG, SSRRPRG_PROGRAM AS COD_PROGRAMAS_COMPARTIDOS
    , SMRPRLE_PROGRAM_DESC AS PROGRAMAS_COMPARTIDOS, SSRRCMP_CAMP_IND AS INCLUIR_EXCLUIR_CAMP, SSRRCMP_CAMP_CODE AS CAMPUS_COMPARTIDOS
    , SSBSECT_PREREQ_CHK_METHOD_CDE AS TIPO_PRERREQUISITO, REQUISITOS, BLOQUES_HORARIOS
FROM (SELECT DISTINCT
	  ...
	  LEFT OUTER JOIN UPN_RPT_DM_PROD.ODSMGR.SSRXLST ON SSBSECT_TERM_CODE = SSRXLST_TERM_CODE AND SSBSECT_CRN = SSRXLST_CRN
      LEFT OUTER JOIN UPN_RPT_DM_PROD.ODSMGR.STVSSTS ON SSBSECT_SSTS_CODE = STVSSTS_CODE
      LEFT OUTER JOIN UPN_RPT_DM_PROD.ODSMGR.SSRMEET ON SSBSECT_TERM_CODE = SSRMEET_TERM_CODE AND SSBSECT_CRN = SSRMEET_CRN
      LEFT OUTER JOIN (SELECT SIRASGN_TERM_CODE, SIRASGN_CRN, SIRASGN_PIDM, SIRASGN_CATEGORY, SIRASGN_PRIMARY_IND
						...
						LEFT OUTER JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_SIBINST ON SIRASGN_PIDM = SIBINST_PIDM AND SIRASGN_TERM_CODE >= SIBINST_TERM_CODE_EFF
      ...
      LEFT OUTER JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_SPRIDEN ON SIRASGN_PIDM = SPRIDEN_PIDM AND SPRIDEN_CHANGE_IND IS NULL
      LEFT OUTER JOIN UPN_RPT_DM_PROD.ODSMGR.STVFCST ON SIBINST_FCST_CODE = STVFCST_CODE
      LEFT OUTER JOIN UPN_RPT_DM_PROD.ODSMGR.STVWKLD ON SIBINST_WKLD_CODE = STVWKLD_CODE
	  LEFT OUTER JOIN UPN_RPT_DM_PROD.ODSMGR.SSRATTR ON SSBSECT_TERM_CODE = SSRATTR_TERM_CODE AND SSBSECT_CRN = SSRATTR_CRN
      LEFT OUTER JOIN UPN_RPT_DM_PROD.ODSMGR.STVATTR ON SSRATTR_ATTR_CODE = STVATTR_CODE
      LEFT OUTER JOIN (SELECT SSRRPRG_TERM_CODE AS SSRRPRG_TERM_CODE_1, SSRRPRG_CRN AS SSRRPRG_CRN_1, SSRRPRG_PROGRAM_IND
	  ...
	  LEFT OUTER JOIN (SELECT SSRRPRG_TERM_CODE AS SSRRPRG_TERM_CODE_2, SSRRPRG_CRN AS SSRRPRG_CRN_2, SSRRPRG_PROGRAM
	  ...
	  LEFT OUTER JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_SMRPRLE ON SSRRPRG_PROGRAM = SMRPRLE_PROGRAM
      LEFT OUTER JOIN (SELECT SSRRCMP_TERM_CODE AS SSRRCMP_TERM_CODE_1, SSRRCMP_CRN AS SSRRCMP_CRN_1, SSRRCMP_CAMP_IND
      ...
      LEFT OUTER JOIN (SELECT SSRRCMP_TERM_CODE AS SSRRCMP_TERM_CODE_2, SSRRCMP_CRN AS SSRRCMP_CRN_2, SSRRCMP_CAMP_CODE
      ...
      LEFT OUTER JOIN (SELECT DISTINCT SSRRTST_TERM_CODE, SSRRTST_CRN
      ...
      LEFT OUTER JOIN (SELECT DISTINCT SSRBLCK_TERM_CODE, SSRBLCK_CRN
      ...
      LEFT OUTER JOIN (SELECT DISTINCT SUBJ_CODE, CRSE_NUMB_LOW, SUBSTR(AREA_COURSE,1,4) AS MAJOR, TERM_CODE_EFF
...
select distinct 
le.STVTERM_CODE s_sem_codigo
from UPN_RPT_DM_PROD.ODSMGR.LOE_STVTERM le
where le.STVTERM_CODE <> '000000' 
and le.STVTERM_CODE <> '999999' 
and YEAR(le.STVTERM_START_DATE)>=2018 
and substr(le.STVTERM_CODE,4,1) in ('3','4','5','6','7','8','9')
order by le.STVTERM_CODE asc
) 
...
CAMPUS,
COD_PROG,
CAST(PROGRAMA AS NVARCHAR(250))PROGRAMA,
MATERIA,
CURSO,
NOMBRE_CURSO,
CICLO,
NRC,
ESTATUS,
CAST(LST_CRZ AS NVARCHAR(50))LST_CRZ,
TIP_HOR,
MET_EDU,
MAXIMO,
REAL,
...
ID_DOCENTE,
DOCENTE,
CAST(TIPO_JORNADA AS NVARCHAR(50))TIPO_JORNADA,
CAST(ESTADO_DOCENTE AS NVARCHAR(50))ESTADO_DOCENTE,
FECHA_INICIO,
FECHA_FIN,
LUNES,
MARTES,
...
SABADO,
DOMINGO,
CAST(HORA AS NVARCHAR(50))HORA,
EDIFICIO,
AULA,
INCLUIR_EXCLUIR_PROG,
COD_PROGRAMAS_COMPARTIDOS,
PROGRAMAS_COMPARTIDOS,
INCLUIR_EXCLUIR_CAMP,
CAMPUS_COMPARTIDOS,
TIPO_PRERREQUISITO
from data