SELECT CAST(SSBSECT_TERM_CODE AS VARCHAR(20)) AS PERIODO, CAST(SSBSECT_CAMP_CODE AS VARCHAR(20)) AS CAMPUS, CAST(SSRATTR_ATTR_CODE AS VARCHAR(20)) AS COD_PROG, CAST(STVATTR_DESC AS VARCHAR(100)) AS PROGRAMA
	, CAST(SSBSECT_SUBJ_CODE || SSBSECT_CRSE_NUMB AS VARCHAR(20)) AS COD_CURSO, CAST(SCBCRSE_TITLE AS VARCHAR(100)) AS NOMBRE_CURSO, CICLO, CAST(SSBSECT_CRN AS VARCHAR(20)) AS NRC, CAST(STVSSTS_DESC AS VARCHAR(20)) AS ESTATUS
	, CAST(SSRXLST_XLST_GROUP AS VARCHAR(20)) AS LST_CRZ, CAST(SSBSECT_SCHD_CODE AS VARCHAR(20)) AS TIP_HOR, CAST(SSBSECT_INSM_CODE AS VARCHAR(20)) AS MET_EDU, SSBSECT_MAX_ENRL AS MAXIMO
	, SSBSECT_ENRL AS REAL, SSBSECT_SEATS_AVAIL AS RESTANTE, SCBCRSE_BILL_HR_LOW AS HRS_CREDITO, CAST(SPRIDEN_ID AS VARCHAR(20)) AS ID_DOCENTE, CAST(DOCENTE_NAME AS VARCHAR(100)) AS DOCENTE
    , SIRASGN_PRIMARY_IND AS ID_PRINC, CAST(STVWKLD_DESC AS VARCHAR(20)) AS TIPO_JORNADA, CAST(STVFCST_DESC AS VARCHAR(100)) AS ESTADO_DOCENTE, SSRMEET_START_DATE AS FECHA_INICIO, SSRMEET_END_DATE AS FECHA_FIN
    ...
    , CAST(CASE WHEN SSRMEET_BEGIN_TIME IS NOT NULL THEN SSRMEET_BEGIN_TIME ||' - '|| SSRMEET_END_TIME END AS VARCHAR(20)) AS HORA, CAST(SSRMEET_BLDG_CODE AS VARCHAR(20)) AS EDIFICIO, CAST(SSRMEET_ROOM_CODE AS VARCHAR(20)) AS AULA
    , SSRRPRG_PROGRAM_IND AS INCLUIR_EXCLUIR_PROG, CAST(SSRRPRG_PROGRAM AS VARCHAR(20)) AS COD_PROGRAMAS_COMPARTIDOS, CAST(SMRPRLE_PROGRAM_DESC AS VARCHAR(100)) AS PROGRAMAS_COMPARTIDOS
    , SSRRCMP_CAMP_IND AS INCLUIR_EXCLUIR_CAMP, CAST(SSRRCMP_CAMP_CODE AS VARCHAR(20)) AS CAMPUS_COMPARTIDOS, CAST(SSBSECT_PREREQ_CHK_METHOD_CDE AS VARCHAR(20)) AS TIPO_PRERREQUISITO
    , CAST(REQUISITOS AS VARCHAR(255)) AS REQUISITOS, CAST(BLOQUES_HORARIOS AS VARCHAR(1024)) AS BLOQUES_HORARIOS
FROM (SELECT DISTINCT
	...
	WHERE SSBSECT_TERM_CODE IN ('220435') --INGRESAR PERIODO
