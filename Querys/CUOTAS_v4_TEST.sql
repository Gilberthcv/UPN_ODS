...
GROUP BY B.PO_REF, A.BILL_TO_CUST_ID, A.INVOICE, A.BILL_TYPE_ID, A.BILL_STATUS, D.ITEM_STATUS
...
SELECT DISTINCT pension.S_SEM_CODIGO,
    ...
    pension.N_CUO_CUOTA,
    (CASE WHEN pension.N_CUO_CUOTA = 0 THEN (CASE WHEN UPPER(pension.S_CUO_CONCEPTO) LIKE '%MATR%' OR pension.EXEMPTION_CROSS_REF IN ('FM','XM','YM','ZM') THEN 'MATRICULA' 
                                                  WHEN UPPER(pension.S_CUO_CONCEPTO) LIKE '%CUOTA%INI%' OR pension.EXEMPTION_CROSS_REF IN ('T1','X1','Y1','Z1') THEN 'CUOTA INICIAL' 
                                                ELSE 'ARANCEL' END) ELSE 'CUOTA ' || pension.N_CUO_CUOTA END) AS S_CUO_CONCEPTO,
...
FROM
    (
      SELECT S_SEM_CODIGO, S_ALU_CODIGO, S_CUO_INVOICE, S_CUO_INVOICE_STATUS, S_CUO_ITEM_STATUS, ENTRY_TYPE, ENTRY_REASON, N_CUO_INVOICE_AMOUNT, N_CUO_BALANCE, D_CUO_FECHA_EMISION, D_CUO_FECHA_VENCIMIENTO, D_CUO_FECHA_PAGO, S_CUO_TIPO_PAGO, N_CUO_CUOTA, S_CUO_CONCEPTO, SUM(N_CUO_MONTO_LINEA) AS MONTO, EXEMPTION_CROSS_REF
      FROM CUOTAS
      ...
      GROUP BY S_SEM_CODIGO, S_ALU_CODIGO, S_CUO_INVOICE, S_CUO_INVOICE_STATUS, S_CUO_ITEM_STATUS, ENTRY_TYPE, ENTRY_REASON, N_CUO_INVOICE_AMOUNT, N_CUO_BALANCE, D_CUO_FECHA_EMISION, D_CUO_FECHA_VENCIMIENTO, D_CUO_FECHA_PAGO, S_CUO_TIPO_PAGO, N_CUO_CUOTA, S_CUO_CONCEPTO, EXEMPTION_CROSS_REF
    ) pension
    left join (
...
WHERE 
    ...
    S_SEM_CODIGO = '219513'
    AND S_ALU_CODIGO = 'N00112075'
    --S_SEM_CODIGO = '219513'
    --AND S_ALU_CODIGO = 'N00198757'
ORDER BY 1,2,3,6;