...
FROM
    (SELECT DISTINCT
        ...
        , MAX(f.ADD_DTTM) OVER (PARTITION BY f.BILL_TO_CUST_ID, f.PO_REF, f.CARGO) AS MAX_ADD_DTTM
    FROM HOLD a
        ,(SELECT 
          b.BUSINESS_UNIT, 
          b.INVOICE, 
    ...
    WHERE a.HOLD = 'C1' AND a.ACTIVE_HOLD_IND = 'Y'
        AND a.ID = f.BILL_TO_CUST_ID AND SUBSTR(a.HOLD_EXPLANATION,1,6) = f.PO_REF AND f.CARGO IS NOT NULL AND f.BILL_TYPE_ID IN ('B1','F1')
        AND SUBSTR(a.HOLD_EXPLANATION,1,6) IN ('219435','219534')) g --INGRESAR PERIODO
