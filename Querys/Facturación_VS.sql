SELECT
    CAST(Z.BUSINESS_UNIT AS VARCHAR(50)) AS BUSINESS_UNIT
    , CAST(Z.PO_REF AS VARCHAR(50)) AS PO_REF
    , CAST(Z.PRODUCT AS VARCHAR(50)) AS PRODUCT
    , CAST(Z.NIVEL AS VARCHAR(50)) AS NIVEL
    , CAST(Z.OPERATING_UNIT AS VARCHAR(50)) AS OPERATING_UNIT
    , CAST(Z.CAMPUS AS VARCHAR(50)) AS CAMPUS
    , CAST(Z.BILL_TO_CUST_ID AS VARCHAR(50)) AS BILL_TO_CUST_ID
    , CAST(REPLACE(Z.NOMBRE_COMPLETO,';','Ñ') AS VARCHAR(255)) AS NOMBRE_COMPLETO
    , CAST(REPLACE(Z.NOMBRE_COMPLETO_CORPORATE,';','Ñ') AS VARCHAR(255)) AS NOMBRE_COMPLETO_CORPORATE
    , CAST(Z.BILL_TYPE_ID AS VARCHAR(50)) AS BILL_TYPE_ID
    , Z.INSTALL_NBR
    , CAST(Z.INVOICE AS VARCHAR(50)) AS INVOICE
    , CAST(Z.INVOICE2 AS VARCHAR(50)) AS INVOICE2
    , Z.LINE_SEQ_NUM
    , CAST(Z.DESCR AS VARCHAR(255)) AS DESCR
    , CAST(Z.SVRSVPR_SRVC_CODE AS VARCHAR(50)) AS SVRSVPR_SRVC_CODE
    , CAST(Z.SVVSRVC_DESC AS VARCHAR(255)) AS SVVSRVC_DESC
    , CAST(Z.SVVSRVC_SRCA_CODE AS VARCHAR(255)) AS SVVSRVC_SRCA_CODE
    , Z.INVOICE_DT
    , Z.FECHA_VENCIMIENTO
    , CAST(Z.VAT_RGSTRN_ID AS VARCHAR(50)) AS VAT_RGSTRN_ID
    , Z.NET_EXTENDED_AMT
    , Z.INVOICE_AMOUNT
    , CAST(Z.ORIGINAL_INVOICE AS VARCHAR(50)) AS ORIGINAL_INVOICE
    , CAST(Z.ORIGINAL_INVOICE2 AS VARCHAR(50)) AS ORIGINAL_INVOICE2
    , CAST(Z.ENTRY_TYPE_BI AS VARCHAR(50)) AS ENTRY_TYPE_BI
    , CAST(Z.ACCOUNT_BI AS VARCHAR(50)) AS ACCOUNT_BI
    , CAST(Z.JOURNAL_ID_BI AS VARCHAR(50)) AS JOURNAL_ID_BI
    , Z.JOURNAL_DATE_BI
    , CAST(Z.REV_RECOG_BASIS_DESC AS VARCHAR(255)) AS REV_RECOG_BASIS_DESC
    , Z.ITEM_SEQ_NUM
    , CAST(Z.ENTRY_TYPE_AR AS VARCHAR(50)) AS ENTRY_TYPE_AR
    , CAST(Z.ENTRY_REASON_AR AS VARCHAR(50)) AS ENTRY_REASON_AR
    , CAST(Z.JOURNAL_ID_AR AS VARCHAR(50)) AS JOURNAL_ID_AR
    , Z.JOURNAL_DATE_AR
    ...
    , Z.MONTO_IMPUESTO
    , (Z.MONTO_BI_AR_CALCULADO + Z.MONTO_IMPUESTO) AS MONTO_TOTAL
    , CAST(NVL(Z.SUBCATEGORIA1,Z.SUBCATEGORIA2) AS VARCHAR(255))AS SUBCATEGORIA
    , CAST(CASE
    	WHEN Z.PERIODO IS NOT NULL AND (NVL(Z.SUBCATEGORIA1,Z.SUBCATEGORIA2) = 'MATRICULA / CUOTA INICIAL' OR SUBSTR(NVL(Z.SUBCATEGORIA1,Z.SUBCATEGORIA2),1,5) = 'CUOTA') THEN NVL(Z.SUBCATEGORIA1,Z.SUBCATEGORIA2)
    ...
    , CAST(CASE WHEN Z.PERIODO IS NOT NULL AND (NVL(Z.SUBCATEGORIA1,Z.SUBCATEGORIA2) = 'MATRICULA / CUOTA INICIAL' OR SUBSTR(NVL(Z.SUBCATEGORIA1,Z.SUBCATEGORIA2),1,5) = 'CUOTA')
    		THEN Z.PERIODO ELSE 'OTROS INGRESOS' END AS VARCHAR(255)) AS PERIODO
    , Z.START_DATE
    , Z.END_DATE
    , CAST(CASE WHEN NVL(Z.SUBCATEGORIA1,Z.SUBCATEGORIA2) IN ('INGRESOS POR PAGO ATRASADO','INTERES MORATORIO','SERVICIO','PAGOS ADELANTADOS')
    			OR (SUBSTR(Z.PO_REF,4,1) IN ('3','4','5') AND TO_CHAR(Z.INVOICE_DT,'YYYY') = TO_CHAR(Z.END_DATE,'YYYY'))
    		THEN 'Y' ELSE Z.ACTIVA END AS VARCHAR(50)) AS ACTIVA
...
FROM ( SELECT DISTINCT 
            CAST(A.BUSINESS_UNIT AS VARCHAR(50)) AS BUSINESS_UNIT, 
            A.INVOICE, A.BILL_STATUS, A.BILL_TYPE_ID, A.INVOICE_AMOUNT, A.INVOICE_DT, A.PO_REF, A.TOT_VAT, A.DUE_DT
        ...
        WHERE A.BUSINESS_UNIT = B.BUSINESS_UNIT AND A.INVOICE = B.INVOICE
            ...
            AND A.BUSINESS_UNIT = 'PER03' --AND A.INVOICE_DT BETWEEN TO_DATE('2019-01-01','YYYY-MM-DD') AND TO_DATE('2020-06-30','YYYY-MM-DD')
            AND A.INVOICE_DT BETWEEN DATE_TRUNC('MONTH',CURRENT_DATE-1) AND (CURRENT_DATE-1)
        GROUP BY A.BUSINESS_UNIT, A.INVOICE, A.BILL_STATUS, A.BILL_TYPE_ID, A.INVOICE_AMOUNT, A.INVOICE_DT, A.PO_REF, A.TOT_VAT, A.DUE_DT
			...
			LEFT JOIN ODSMGR.PS_BI_ACCT_ENTRY E ON D.BUSINESS_UNIT = E.BUSINESS_UNIT AND D.INVOICE = E.INVOICE AND D.LINE_SEQ_NUM = E.LINE_SEQ_NUM
													...
													--AND E.JOURNAL_DATE BETWEEN TO_DATE('2019-01-01','YYYY-MM-DD') AND TO_DATE('2020-06-30','YYYY-MM-DD')
													AND E.JOURNAL_DATE BETWEEN DATE_TRUNC('MONTH',CURRENT_DATE-1) AND (CURRENT_DATE-1)
			LEFT JOIN ( SELECT DISTINCT F.BUSINESS_UNIT, F.ITEM, F.ITEM_LINE, F.ITEM_SEQ_NUM, F.ENTRY_TYPE, F.ENTRY_REASON, F.ENTRY_AMT, F.ACCOUNTING_DT, G.JOURNAL_ID, G.JOURNAL_DATE
				        FROM (SELECT BUSINESS_UNIT, ITEM, ITEM_LINE, ITEM_SEQ_NUM, ENTRY_TYPE, ENTRY_REASON, ENTRY_AMT, ACCOUNTING_DT
				                ...
				                WHERE BUSINESS_UNIT = 'PER03' AND ENTRY_TYPE NOT IN ('PY','MT') --AND ACCOUNTING_DT BETWEEN TO_DATE('2019-01-01','YYYY-MM-DD') AND TO_DATE('2020-06-30','YYYY-MM-DD')) F
				                	AND ACCOUNTING_DT BETWEEN DATE_TRUNC('MONTH',CURRENT_DATE-1) AND (CURRENT_DATE-1)) F
				        ...
				        WHERE F.BUSINESS_UNIT = G.BUSINESS_UNIT(+) AND F.ITEM = G.ITEM(+) AND F.ITEM_LINE = G.ITEM_LINE(+) AND F.ITEM_SEQ_NUM = G.ITEM_SEQ_NUM(+)
				            ...
				            AND G.LEDGER(+) = 'ACTUALS' --AND G.JOURNAL_DATE(+) BETWEEN TO_DATE('2019-01-01','YYYY-MM-DD') AND TO_DATE('2020-06-30','YYYY-MM-DD') ) H ON 
				            AND G.JOURNAL_DATE(+) BETWEEN DATE_TRUNC('MONTH',CURRENT_DATE-1) AND (CURRENT_DATE-1) ) H ON
