...
WITH PERIODO_ACADEMICO AS (
	...
	FROM (SELECT PC.TERM_CODE AS PERIODO_ACTUAL, SUBSTR(STVTERM_DESC,1,6) AS PERIODO_ACTUAL_LBL, STVTERM_DESC AS PERIODO_ACTUAL_DESC
	    ...
	    FROM LOE_SECTION_PART_OF_TERM PC, UPN_RPT_DM_PROD.ODSMGR.STVTERM, LOE_SECTION_PART_OF_TERM PP
	    ...
	    , UPN_RPT_DM_PROD.ODSMGR.STVTERM D, LOE_SECTION_PART_OF_TERM F
...
FROM (((SELECT SHRTCKN_PIDM, SPRIDEN_ID, MAX(MAX_PERIODO_HISTORIA) AS MAX_PERIODO_HISTORIA, PERIODO_ACTUAL_LBL
		FROM ( SELECT SHRTCKN_PIDM, SPRIDEN_ID, MAX(SHRTCKN_TERM_CODE) AS MAX_PERIODO_HISTORIA, PERIODO_ACTUAL_LBL
				FROM SHRTCKN, SHRTCKL, LOE_SHRTCKG G, LOE_SPRIDEN, PERIODO_ACADEMICO
	            ...
	            FROM UPN_RPT_DM_PROD.ODSMGR.STUDENT_COURSE, PERIODO_ACADEMICO
		...
		GROUP BY SHRTCKN_PIDM, SPRIDEN_ID, PERIODO_ACTUAL_LBL
        	...
        	LEFT JOIN ( SELECT PERSON_UID, ID, MAX_PERIODO_EGRESO, PERIODO_ACTUAL_LBL
						...
						WHERE MAX_PERIODO_EGRESO <= PERIODO_ANTERIOR
						) EGR ON HST.SHRTCKN_PIDM = EGR.PERSON_UID AND HST.PERIODO_ACTUAL_LBL = EGR.PERIODO_ACTUAL_LBL)
			LEFT JOIN LOE_SECTION_PART_OF_TERM TERM1 ON HST.MAX_PERIODO_HISTORIA = TERM1.TERM_CODE
			LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.STVTERM TERM2 ON HST.MAX_PERIODO_HISTORIA = TERM2.STVTERM_CODE)
