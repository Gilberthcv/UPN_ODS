...
SELECT
    Z.BUSINESS_UNIT, Z.PO_REF, Z.PRODUCT, Z.OPERATING_UNIT, Z.BILL_TO_CUST_ID, REPLACE(Z.NOMBRE_COMPLETO,';','Ñ') AS NOMBRE_COMPLETO
    , REPLACE(Z.NOMBRE_COMPLETO_CORPORATE,';','Ñ') AS NOMBRE_COMPLETO_CORPORATE
...
SELECT DISTINCT
    ...
    , T.SEMESTRE ||' ('|| T.NIVEL ||')' AS PERIODO
    , U.START_DATE, U.END_DATE, CASE WHEN D.INVOICE_DT > U.END_DATE THEN 'N' ELSE 'Y' END AS ACTIVA
        ...
        GROUP BY A.BUSINESS_UNIT, A.INVOICE, A.BILL_STATUS, A.BILL_TYPE_ID, A.INVOICE_AMOUNT, A.INVOICE_DT, A.PO_REF, A.TOT_VAT, A.DUE_DT
			...
			LEFT JOIN ( SELECT TERM_CODE, SUBSTR(STVTERM_DESC,1,6) AS SEMESTRE, STVTERM_DESC
						...
						WHERE TERM_CODE = STVTERM_CODE AND WEEKS > 15 AND WEEKS < 18
							AND SUBSTR(TERM_CODE,4,1) IN ('4','5') AND TERM_CODE > 217430 ) T ON
											D.PO_REF = T.TERM_CODE
			
			LEFT JOIN ODSMGR.LOE_SECTION_PART_OF_TERM U ON D.PO_REF = U.TERM_CODE
