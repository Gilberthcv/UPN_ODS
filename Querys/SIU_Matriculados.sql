...
SELECT DISTINCT A.PERSON_UID, A.ID, A.STUDENT_LEVEL, CASE WHEN H.MAESTRIA IS NOT NULL THEN A.PROGRAM ||' - '|| H.MAESTRIA ELSE A.PROGRAM END AS PROGRAMA
	...
	, TO_CHAR(F.START_DATE,'DD/MM/YYYY') AS FECHA_CAMBIO_PROGRAMA
	, CASE
	...
	, TO_CHAR(MIN(B.SECTION_ADD_DATE) OVER(PARTITION BY B.PERSON_UID, B.ACADEMIC_PERIOD),'DD/MM/YYYY') AS FECHA_MATRICULA
	, NULL AS CODIGO_ESCALA_PAGO, NULL AS PORCENTAJE_DESCUENTO
	, CASE WHEN (J.PERSON_UID IS NOT NULL AND M.TBBCSTU_STU_PIDM IS NOT NULL) OR (K.TBBESTU_PIDM IS NOT NULL AND L.EXEMPTION_CODE IS NOT NULL) THEN 1 ELSE 0 END AS TIENE_BECA
	, CASE
		WHEN J.STUDENT_ATTRIBUTE = 'BK18' AND M.TBBCSTU_STU_PIDM IS NOT NULL THEN 1
		WHEN K.TBBESTU_PIDM IS NOT NULL AND L.EXEMPTION_CODE IS NOT NULL THEN 2
		WHEN J.STUDENT_ATTRIBUTE IN ('FSMI','CONT') AND M.TBBCSTU_STU_PIDM IS NOT NULL THEN 3
		ELSE NULL END AS TIPO_BECA
	, CASE
		WHEN (J.STUDENT_ATTRIBUTE = 'BK18' AND M.TBBCSTU_STU_PIDM IS NOT NULL) OR K.TBBESTU_PIDM IS NOT NULL THEN NULL
		WHEN J.STUDENT_ATTRIBUTE = 'FSMI' AND M.TBBCSTU_STU_PIDM IS NOT NULL THEN 'FONDO SOCIAL MICHIQUILLAY'
		WHEN J.STUDENT_ATTRIBUTE = 'CONT' AND M.TBBCSTU_STU_PIDM IS NOT NULL THEN 'CONTRATOS EMPRESAS'
		ELSE NULL END AS OTRA_BECA
	, CASE
		WHEN J.STUDENT_ATTRIBUTE = 'BK18' AND M.TBBCSTU_STU_PIDM IS NOT NULL THEN NULL
		WHEN K.TBBESTU_PIDM IS NOT NULL AND L.EXEMPTION_CODE IS NOT NULL THEN L.CHARGE_PERCENTAGE
		WHEN J.STUDENT_ATTRIBUTE = 'FSMI' AND M.TBBCSTU_STU_PIDM IS NOT NULL THEN 100
		WHEN J.STUDENT_ATTRIBUTE = 'CONT' AND M.TBBCSTU_STU_PIDM IS NOT NULL THEN NULL
...
FROM ODSMGR.ACADEMIC_STUDY A
		...
		LEFT JOIN ODSMGR.LOE_EXEMPTION_DETAIL_LEVEL L ON K.TBBESTU_TERM_CODE = L.EXEMPTION_TERM AND K.TBBESTU_EXEMPTION_CODE = L.EXEMPTION_CODE AND SUBSTR(L.EXEMPTION_CODE,5,1) = '1' AND SUBSTR(L.REGISTRATION_DETAIL_CODE,1,2) LIKE '%A'
		LEFT JOIN ODSMGR.LOE_TBBCSTU M ON J.PERSON_UID = M.TBBCSTU_STU_PIDM AND J.ACADEMIC_PERIOD = M.TBBCSTU_TERM_CODE AND M.TBBCSTU_DEL_IND IS NULL
...
WHERE A.ENROLLMENT_STATUS IN ('EL','RF','RO','SE')
	AND ((A.STUDENT_LEVEL ='UG' AND A.ACADEMIC_PERIOD IN ('220413','220513'))
		...
		OR (A.STUDENT_LEVEL = 'MA' AND A.ACADEMIC_PERIOD IN ('218816','219813','219839','220813') AND (A.ACADEMIC_PERIOD = C.SOVLCUR_TERM_CODE_ADMIT OR C.SOVLCUR_TERM_CODE_ADMIT IS NULL)))
;
