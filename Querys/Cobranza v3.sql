...
FROM ODSMGR.PS_BI_HDR A
		...
		LEFT JOIN ( SELECT DISTINCT ACCOUNT_UID, ID, NAME, CREDIT_CARD_NUMBER, SVRSVPR_SRVC_CODE, SVVSRVC_DESC, SVVSRVC_SRCA_CODE
					...
					) D ON B.INVOICE = D.CREDIT_CARD_NUMBER AND B.DESCR = D.SVVSRVC_DESC
...
--SELECT CAST(ACCOUNTING_DT AS DATE) AS FECHA_COBRANZA, SUM(MONETARY_AMOUNT) AS MONTO_PAGO FROM (
SELECT DISTINCT
	...
	, D.BILL_TO_CUST_ID, REPLACE(REPLACE(CONCAT(CONCAT(J.NAME2,' '),J.NAME1),'/',' '),';','Ñ') AS NOMBRE_COMPLETO, D.BILL_TYPE_ID, D.INVOICE, K.INVOICE2
...
FROM ODSMGR.PS_ITEM A
		INNER JOIN ODSMGR.PS_ITEM_DST B ON A.BUSINESS_UNIT = B.BUSINESS_UNIT AND A.ITEM = B.ITEM AND A.ITEM_LINE = B.ITEM_LINE AND B.LEDGER = 'ACTUALS' AND B.ACCOUNT NOT IN ('1140000','4000000')
		...
		LEFT JOIN ODSMGR.PS_LI_PER_ARPMT_VW E ON C.BUSINESS_UNIT = E.DEPOSIT_BU AND C.DEPOSIT_ID = E.DEPOSIT_ID AND C.PAYMENT_ID = E.PAYMENT_ID AND C.PAYMENT_SEQ_NUM = E.PAYMENT_SEQ_NUM
												AND C.ITEM = E.INVOICE AND C.CUST_ID = E.CUST_ID
		LEFT JOIN ODSMGR.PS_LI_GBL_ARPY_DTL F ON E.DEPOSIT_BU = F.DEPOSIT_BU AND E.PYMNT_REF_ID = F.PYMNT_REF_ID
		LEFT JOIN ODSMGR.PS_PAYMENT G ON C.BUSINESS_UNIT = G.DEPOSIT_BU AND C.DEPOSIT_ID = G.DEPOSIT_ID AND C.PAYMENT_SEQ_NUM = G.PAYMENT_SEQ_NUM
		...
		LEFT JOIN ( SELECT TERM_CODE, SUBSTR(STVTERM_DESC,1,6) AS SEMESTRE, STVTERM_DESC, CASE SUBSTR(TERM_CODE,4,1) WHEN '4' THEN 'UG' WHEN '5' THEN 'WA' END AS NIVEL
...
--) GROUP BY CAST(ACCOUNTING_DT AS DATE) ORDER BY 1
;
