...
WITH CUOTAS AS (
	SELECT A.PO_REF AS S_SEM_CODIGO, 
	    ...
	    MAX(CASE WHEN I.TEXT254 LIKE '%EXEMPTION/CROSS_REF:%' OR I.NOTE_TYPE = 'EXMPT_CODE' THEN SUBSTR(I.TEXT254,LENGTH(I.TEXT254)-3,2) END) AS EXEMPTION_CROSS_REF
	FROM UPN_RPT_DM_PROD.ODSMGR.PS_BI_HDR A
	    ...
	    LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_ITEM_ACTIVITY Y ON A.INVOICE = Y.ITEM
	    														AND Y.ITEM_SEQ_NUM = ( SELECT MAX(Y_MAX.ITEM_SEQ_NUM)
	    ...
	    LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_PAY_TRMS_NET E ON E.PYMNT_TERMS_CD = A.PYMNT_TERMS_CD
	    														AND E.EFFDT = ( SELECT MAX(E_ED.EFFDT)
																				FROM UPN_RPT_DM_PROD.ODSMGR.PS_PAY_TRMS_NET E_ED
																				WHERE E.SETID = E_ED.SETID AND E.PYMNT_TERMS_CD = E_ED.PYMNT_TERMS_CD AND E_ED.EFFDT <= CURRENT_DATE )
	    LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_BI_LINE_NOTE G ON A.BUSINESS_UNIT = G.BUSINESS_UNIT
	    														AND A.INVOICE = CASE
	    																			WHEN SUBSTR(G.TEXT254,1,17) = 'ORIGINAL_INVOICE:' THEN SUBSTR(G.TEXT254,19,11)
	    																			WHEN G.NOTE_TYPE = 'INVOICE' THEN G.TEXT254
	    																			ELSE ' ' END
	    ...
	    LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_BI_INSTALL_SCHE J ON I.BUSINESS_UNIT = J.BUSINESS_UNIT
	    														AND J.GENERATED_INVOICE = CASE
	    																					WHEN SUBSTR(I.TEXT254,1,17) = 'ORIGINAL_INVOICE:' THEN SUBSTR(I.TEXT254,19,11)
	    																					WHEN I.NOTE_TYPE = 'INVOICE' THEN I.TEXT254
	    																					ELSE ' ' END
	    ...
	    LEFT JOIN ( SELECT DISTINCT PAY.PAYMENT_SEQ_NUM, PAY.PAYMENT_ID, PAY.ACCOUNTING_DT , PAY.PAYMENT_METHOD ,PAY_I.DEPOSIT_ID, PAY_I.REF_VALUE
	                FROM UPN_RPT_DM_PROD.ODSMGR.PS_PAYMENT PAY, UPN_RPT_DM_PROD.ODSMGR.PS_PAYMENT_ID_ITEM PAY_I
	                WHERE PAY.DEPOSIT_BU = PAY_I.DEPOSIT_BU AND PAY.PAYMENT_SEQ_NUM = PAY_I.PAYMENT_SEQ_NUM AND PAY.DEPOSIT_ID = PAY_I.DEPOSIT_ID
...
FROM (
	...
	FROM
	    (
	      SELECT S_SEM_CODIGO, S_ALU_CODIGO, S_CUO_INVOICE, S_CUO_INVOICE_STATUS, S_CUO_ITEM_STATUS, ENTRY_TYPE, ENTRY_REASON, N_CUO_INVOICE_AMOUNT, N_CUO_BALANCE, D_CUO_FECHA_EMISION, D_CUO_FECHA_VENCIMIENTO, D_CUO_FECHA_PAGO, S_CUO_TIPO_PAGO, N_CUO_CUOTA, EXEMPTION_CROSS_REF
			  , CASE WHEN SUBSTR(S_CUO_CONCEPTO,1,2) = 'FM' OR UPPER(S_CUO_CONCEPTO) LIKE '%MATR%' OR EXEMPTION_CROSS_REF IN ('FM','XM','YM','ZM') THEN 'MATRICULA' 
                ...
                ELSE 'ARANCEL' END AS S_CUO_CONCEPTO, SUM(CASE WHEN EXEMPTION_CROSS_REF IN ('FM','XM','YM','ZM','T1','X1','Y1','Z1') THEN 0 ELSE N_CUO_MONTO_LINEA END) AS MONTO
	      FROM CUOTAS
	    ...
	    left join (
	      SELECT S_SEM_CODIGO, S_ALU_CODIGO, S_CUO_INVOICE, SUM(N_CUO_MONTO_LINEA) AS MONTO
	      	  , CASE WHEN EXEMPTION_CROSS_REF IN ('FM','XM','YM','ZM') THEN 'MATRICULA' 
				  	WHEN EXEMPTION_CROSS_REF IN ('T1','X1','Y1','Z1') THEN 'CUOTA INICIAL' 
                ELSE 'ARANCEL' END AS S_CUO_CONCEPTO
	      FROM CUOTAS
	      ...
	      GROUP BY S_SEM_CODIGO, S_ALU_CODIGO, S_CUO_INVOICE
	      	  , CASE WHEN EXEMPTION_CROSS_REF IN ('FM','XM','YM','ZM') THEN 'MATRICULA' 
				  	WHEN EXEMPTION_CROSS_REF IN ('T1','X1','Y1','Z1') THEN 'CUOTA INICIAL' 
                ELSE 'ARANCEL' END
	    ) dscto ON pension.S_SEM_CODIGO = dscto.S_SEM_CODIGO AND pension.S_ALU_CODIGO = dscto.S_ALU_CODIGO AND pension.S_CUO_INVOICE = dscto.S_CUO_INVOICE AND pension.S_CUO_CONCEPTO = dscto.S_CUO_CONCEPTO
	    left join (
	      SELECT S_SEM_CODIGO, S_ALU_CODIGO, S_CUO_INVOICE, SUM(N_CUO_MONTO_LINEA) AS MONTO
	      	  , CASE WHEN EXEMPTION_CROSS_REF IN ('FM','XM','YM','ZM') THEN 'MATRICULA' 
				  	WHEN EXEMPTION_CROSS_REF IN ('T1','X1','Y1','Z1') THEN 'CUOTA INICIAL' 
	      ...
	      GROUP BY S_SEM_CODIGO, S_ALU_CODIGO, S_CUO_INVOICE
	      	  , CASE WHEN EXEMPTION_CROSS_REF IN ('FM','XM','YM','ZM') THEN 'MATRICULA' 
				  	WHEN EXEMPTION_CROSS_REF IN ('T1','X1','Y1','Z1') THEN 'CUOTA INICIAL' 
                ELSE 'ARANCEL' END
	    ) beca ON pension.S_SEM_CODIGO = beca.S_SEM_CODIGO AND pension.S_ALU_CODIGO = beca.S_ALU_CODIGO AND pension.S_CUO_INVOICE = beca.S_CUO_INVOICE AND pension.S_CUO_CONCEPTO = beca.S_CUO_CONCEPTO	
	UNION	
	...
	--ORDER BY 1,2,3,6
  ) X --WHERE X.S_ALU_CODIGO = 'N00215394' AND X.S_SEM_CODIGO IN ('220435','220534')
;