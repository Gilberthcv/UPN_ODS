...
SELECT DISTINCT
    ...
    , CASE SUBSTR(a.ACADEMIC_PERIOD,4,1) 
      ...
      ELSE NULL END AS codigo_tipo_periodo_academico, a.COURSE_CAMPUS AS id_campus
    , CASE a.INSTRUCTION_DELIVERY_MODE 
    ...
    , CASE a.INSTRUCTION_DELIVERY_MODE 
      ...
      ELSE NULL END AS id_tipo_impartir, a.BUILDING || '.' || a.ROOM AS id_recurso
    , REPLACE(REPLACE(SUBSTR(c.SLBRDEF_DESC,1,INSTR(c.SLBRDEF_DESC,' ',1,1)),'.',''),',','') AS id_tipo_recurso
    , b.MAXIMUM_ENROLLMENT AS cupo_academico, c.SLBRDEF_CAPACITY AS capacidad_recurso
    , CASE 
          WHEN a.MONDAY_IND = 'M' THEN 1 --Lunes
          WHEN a.TUESDAY_IND = 'T' THEN 2 --Martes
          WHEN a.WEDNESDAY_IND = 'W' THEN 3 --Miercoles
          WHEN a.THURSDAY_IND = 'R' THEN 4 --Jueves
          WHEN a.FRIDAY_IND = 'F' THEN 5 --Viernes
          WHEN a.SATURDAY_IND = 'S' THEN 6 --Sabado
          WHEN a.SUNDAY_IND = 'U' THEN 7 --Domingo
      ELSE NULL END AS dia_programacion
    , SUBSTR(a.BEGIN_TIME,1,2) || ':' || SUBSTR(a.BEGIN_TIME,3,2) AS hora_inicio, SUBSTR(a.END_TIME,1,2) || ':' || SUBSTR(a.END_TIME,3,2) AS hora_fin
    , a.FECHA AS fecha_evento, NULL AS id_sesion, NULL AS desc_sesion
FROM 
    ...
    , BUILDING, ROOM, COURSE_CAMPUS, SCHEDULE, SCHEDULE_DESC, MONDAY_IND, TUESDAY_IND, WEDNESDAY_IND, THURSDAY_IND, FRIDAY_IND, SATURDAY_IND, SUNDAY_IND FROM MEETING_TIME) a
    , SCHEDULE_OFFERING b, SLBRDEF c
WHERE a.ACADEMIC_PERIOD = b.ACADEMIC_PERIOD AND a.COURSE_REFERENCE_NUMBER = b.COURSE_REFERENCE_NUMBER 
    AND a.BUILDING = c.SLBRDEF_BLDG_CODE AND a.ROOM = c.SLBRDEF_ROOM_NUMBER 
    AND a.ACADEMIC_PERIOD IN ('218413','218512') AND a.COURSE_CAMPUS IN ('TML','TSI');
