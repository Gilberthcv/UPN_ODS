...
WITH PER_AR_COBRANZAS AS (
    ...
    WHERE ( ( A.BILL_TO_CUST_ID = C.CUST_ID 
        ...
        AND H.ACCOUNTING_DT BETWEEN TO_DATE('2019-01-01','YYYY-MM-DD') AND TO_DATE('2019-12-31','YYYY-MM-DD') ))	--450,427
        AND J.SOURCE = 'POS'
    )
    , PS AS (
    SELECT DISTINCT A.*
    ...
    FROM PER_AR_COBRANZAS A 
            LEFT JOIN PS_BI_LINE B ON
                ...
                AND (UPPER(B.DESCR) LIKE '%ARANCEL CONTADO%' OR UPPER(B.DESCR) LIKE '%ARANCEL%' 
                    ...
                    OR UPPER(B.DESCR) LIKE '%2DA%' OR UPPER(B.DESCR) LIKE '%3RA%')	--450,427
    WHERE A.TOTAL_PAGADO <> 0	--258,848
    )
    , CURRICULA AS (
...
SELECT CLIENTE
    ...
    , SOVLCUR_STYP_CODE
    , COHORT*/    
FROM PS, SPRIDEN, CURRICULA C, STUDENT_COHORT
WHERE CLIENTE = SPRIDEN_ID AND SPRIDEN_CHANGE_IND IS NULL
    AND SPRIDEN_PIDM = SOVLCUR_PIDM(+)
    AND PO_REF >= SOVLCUR_TERM_CODE(+) AND PO_REF < NVL(SOVLCUR_TERM_CODE_END(+),'999996')
    AND (C.SOVLCUR_SEQNO = (SELECT MAX(C1.SOVLCUR_SEQNO) FROM CURRICULA C1
							WHERE C1.SOVLCUR_PIDM = C.SOVLCUR_PIDM
								AND PO_REF >= C1.SOVLCUR_TERM_CODE AND PO_REF < NVL(C1.SOVLCUR_TERM_CODE_END,'999996'))
			OR C.SOVLCUR_SEQNO IS NULL)
    AND SOVLCUR_PIDM = PERSON_UID(+) AND PO_REF = ACADEMIC_PERIOD(+)
