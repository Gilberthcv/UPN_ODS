...
SELECT A.SOVLCUR_PIDM, SPRIDEN_ID, SPRIDEN_FIRST_NAME, SPRIDEN_LAST_NAME, A.SOVLCUR_CAMP_CODE, A.SOVLCUR_PROGRAM, B.SMRPRLE_PROGRAM_DESC, C.SOVLCUR_PROGRAM, D.SMRPRLE_PROGRAM_DESC
    ...
    , CASE WHEN E1.PERSON_UID IS NOT NULL THEN E1.CURRICULUM_CHANGE_REASON WHEN C.SOVLCUR_PIDM IS NOT NULL THEN 'ACTIVO' END AS STATUS
...
FROM ( SELECT DISTINCT SOVLCUR_PIDM, SOVLCUR_CAMP_CODE, SOVLCUR_PROGRAM, SHRTCKN_TERM_CODE, SHRTCKN_SUBJ_CODE, SHRTCKN_CRSE_NUMB, SHRTCKN_CRSE_TITLE, SHRTCKG_GRDE_CODE_FINAL
        ...
        LEFT JOIN (SELECT DISTINCT PERSON_UID, ID, NAME, STUDENT_LEVEL, PROGRAM, CURRICULUM_CHANGE_REASON
					...
					WHERE SOURCE = 'OUTCOME' AND CURRICULUM_CHANGE_REASON IN ('BACHILLER','TITULADO') AND STUDENT_LEVEL = 'UG') E ON C.SOVLCUR_PIDM = E.PERSON_UID AND C.SOVLCUR_PROGRAM = E.PROGRAM
		LEFT JOIN (SELECT DISTINCT PERSON_UID, ID, NAME, STUDENT_LEVEL, PROGRAM, CURRICULUM_CHANGE_REASON
					FROM ODSMGR.FIELD_OF_STUDY
					WHERE SOURCE = 'OUTCOME' AND CURRICULUM_CHANGE_REASON = 'EGRESADO' AND STUDENT_LEVEL = 'UG') E1 ON C.SOVLCUR_PIDM = E1.PERSON_UID AND C.SOVLCUR_PROGRAM = E1.PROGRAM
...
GROUP BY A.SOVLCUR_PIDM, SPRIDEN_ID, SPRIDEN_FIRST_NAME, SPRIDEN_LAST_NAME, A.SOVLCUR_CAMP_CODE, A.SOVLCUR_PROGRAM, B.SMRPRLE_PROGRAM_DESC, C.SOVLCUR_PROGRAM, D.SMRPRLE_PROGRAM_DESC
    ...
    , CASE WHEN E1.PERSON_UID IS NOT NULL THEN E1.CURRICULUM_CHANGE_REASON WHEN C.SOVLCUR_PIDM IS NOT NULL THEN 'ACTIVO' END
...
FROM ODSMGR.LOE_SOVLCUR
        ...
        LEFT JOIN (SELECT DISTINCT PERSON_UID, ID, NAME, STUDENT_LEVEL, PROGRAM, CURRICULUM_CHANGE_REASON
					FROM ODSMGR.FIELD_OF_STUDY
					WHERE SOURCE = 'OUTCOME' AND CURRICULUM_CHANGE_REASON IN ('BACHILLER','TITULADO') AND STUDENT_LEVEL = 'UG') E ON SOVLCUR_PIDM = E.PERSON_UID AND SOVLCUR_PROGRAM = E.PROGRAM
		LEFT JOIN (SELECT DISTINCT PERSON_UID, ID, NAME, STUDENT_LEVEL, PROGRAM, CURRICULUM_CHANGE_REASON
					FROM ODSMGR.FIELD_OF_STUDY
					WHERE SOURCE = 'OUTCOME' AND CURRICULUM_CHANGE_REASON = 'EGRESADO' AND STUDENT_LEVEL = 'UG') E1 ON SOVLCUR_PIDM = E1.PERSON_UID AND SOVLCUR_PROGRAM = E1.PROGRAM
...
WHERE SOVLCUR_LMOD_CODE = 'LEARNER' AND SOVLCUR_CACT_CODE = 'ACTIVE' AND SOVLCUR_CURRENT_IND = 'Y'
    AND SOVLCUR_LEVL_CODE = 'UG' AND SHRTCKN_PIDM IS NULL AND E.PERSON_UID IS NULL
