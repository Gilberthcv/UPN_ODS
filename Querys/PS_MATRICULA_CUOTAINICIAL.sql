...
FROM (SELECT C.BUSINESS_UNIT, C.INVOICE, C.BILL_TO_CUST_ID, C.BILL_STATUS, C.BILL_TYPE_ID, C.INVOICE_AMOUNT, C.INVOICE_DT
      ...
      FROM ((SELECT A.BUSINESS_UNIT, A.INVOICE, A.BILL_TO_CUST_ID, A.BILL_STATUS, A.BILL_TYPE_ID, A.INVOICE_AMOUNT
             ...
             FROM (PS_BI_HDR A
                    INNER JOIN PS_BI_LINE B ON A.BUSINESS_UNIT = B.BUSINESS_UNIT AND A.INVOICE = B.INVOICE)
                    LEFT JOIN PS_BI_LINE_NOTE C ON B.BUSINESS_UNIT = C.BUSINESS_UNIT AND B.INVOICE = C.INVOICE AND B.LINE_SEQ_NUM = C.LINE_SEQ_NUM
            ...
            GROUP BY A.BUSINESS_UNIT, A.INVOICE, A.BILL_TO_CUST_ID, A.BILL_STATUS, A.BILL_TYPE_ID, A.INVOICE_AMOUNT
              ...
              LEFT JOIN PS_ITEM_ACTIVITY I ON
      ...
      WHERE C.CARGO IN ('CONTADO','CUOTA INICIAL','MATRICULA') AND (D.DEDUCTION_STATUS <> 'REGU' OR D.DEDUCTION_STATUS IS NULL) AND I.ITEM IS NULL) E
      , PS_LI_GBL_ARPY_REF F, PS_LI_GBL_ARPY_TBL G, PS_CDR_RECEIPT H, LOE_SPRIDEN
