...
SELECT DISTINCT NVL(TO_CHAR(A.INVOICE_DT, 'YYYYMMDD'), '-1') TRAN_DATE_DIMKEY
    , CASE
    WHEN B.PRODUCT = '52' THEN '50'
    WHEN B.PRODUCT = ' ' THEN 'NULL_LEVEL'
    WHEN B.PRODUCT IS NULL THEN 'NULL_LEVEL'
    ELSE PRODUCT
    END AS LEVEL_DIMKEY, NVL(A.PO_REF, 'NULL_PERIOD') PERIOD_DIMKEY, CASE
    WHEN B.OPERATING_UNIT <> ' ' THEN B.OPERATING_UNIT
    ELSE 'NULL_CAMPUS'
    END AS CAMPUS_DIMKEY
    , A.BILL_TYPE_ID BILL_TYPE_DIMKEY, A.BILL_STATUS BILL_STATUS_DIMKEY , INVOICE_AMT_PRETAX AS MONTO_AFECTO , INVOICE_AMOUNT AS MONTO_TOTAL , TOT_VAT AS MONTO_IMPUESTO , A.INVOICE
FROM PS_BI_HDR A
    JOIN PS_BI_LINE_DST B ON
        B.INVOICE = A.INVOICE
WHERE INVOICE_DT BETWEEN '01/05/2019' AND '31/05/2019');
