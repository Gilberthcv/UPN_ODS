...
SELECT 
	CAST(I.OPERATING_UNIT AS VARCHAR(255)) AS OPERATING_UNIT, CAST(N.VALUE2 AS VARCHAR(255)) AS CAMPUS, CAST(I.PRODUCT AS VARCHAR(255)) AS PRODUCT, CAST(O.VALUE2 AS VARCHAR(255)) AS NIVEL
	, CAST(D.PO_REF AS VARCHAR(255)) AS PO_REF, CAST(CASE WHEN SUBSTR(E.DRAWER_ID,1,1) = 'C' OR E.PAYMENT_METHOD_CDR = 'CSH' THEN 'CAJA' ELSE 'BANCO' END AS VARCHAR(255)) AS ORIGEN
	, CAST(D.BILL_TO_CUST_ID AS VARCHAR(255)) AS BILL_TO_CUST_ID, CAST(REPLACE(REPLACE(CONCAT(CONCAT(J.NAME2,' '),J.NAME1),'/',' '),';','Ñ') AS VARCHAR(255)) AS NOMBRE_COMPLETO
	, CAST(D.BILL_TYPE_ID AS VARCHAR(255)) AS BILL_TYPE_ID, CAST(D.INVOICE AS VARCHAR(255)) AS INVOICE, CAST(K.INVOICE2 AS VARCHAR(255)) AS INVOICE2, CAST(NVL(E.PAYMENT_METHOD_CDR,G.PAYMENT_METHOD) AS VARCHAR(255)) AS FORMA_PAGO
	, CAST(NVL(CASE E.PAYMENT_METHOD_CDR
		...
		, CASE G.PAYMENT_METHOD
			WHEN 'CSH' THEN 'Efectivo'
			WHEN 'EFT' THEN 'Asbanc'
			ELSE G.PAYMENT_METHOD END) AS VARCHAR(255)) AS FORMA_PAGO_DESC
	, CAST(CASE WHEN F.CR_CARD_TYPE = ' ' OR F.CR_CARD_TYPE IS NULL THEN NULL ELSE F.CR_CARD_TYPE END AS VARCHAR(255)) AS TIPO_TARJETA
	, CAST(CASE
		...
		ELSE F.CR_CARD_TYPE END AS VARCHAR(255)) AS TIPO_TARJETA_DESC
	, CAST(C.DEPOSIT_ID AS VARCHAR(255)) AS DEPOSIT_ID, CAST(C.PAYMENT_ID AS VARCHAR(255)) AS PAYMENT_ID, CAST(E.DRAWER_ID AS VARCHAR(255)) AS DRAWER_ID
	, CAST(TO_CHAR(NVL(E.CREATEDTTM,C.ACCOUNTING_DT),'YYYY-MM-DD HH24:MI:SS') AS DATETIME) AS FECHA_COBRANZA, C.ACCOUNTING_DT, C.POST_DT, D.INVOICE_DT
	...
	, CAST(CASE WHEN B.MONETARY_AMOUNT = D.INVOICE_AMOUNT THEN 'TOTAL' ELSE 'PARCIAL' END AS VARCHAR(255)) AS PAGO
	, CAST(CASE WHEN G.PAYMENT_STATUS = 'C' AND G.WO_ADJ_USED_SW = 'Y' THEN 'Y' ELSE 'N' END AS VARCHAR(255)) AS ANULADO
	, CAST(CASE WHEN G.UNPOST_REASON = 'ERROR' THEN 'Y' ELSE 'N' END AS VARCHAR(255)) AS DESCONTABILIZADO
	, CAST(H.RECON_ID AS VARCHAR(255)) AS RECON_ID, CAST(G.BNK_ID_NBR AS VARCHAR(255)) AS BNK_ID_NBR, CAST(G.BANK_ACCOUNT_NUM AS VARCHAR(255)) AS BANK_ACCOUNT_NUM, CAST(B.ACCOUNT AS VARCHAR(255)) AS ACCOUNT
	, CAST(B.JOURNAL_ID AS VARCHAR(255)) AS JOURNAL_ID, B.JOURNAL_DATE
	, CAST(CASE WHEN B.JOURNAL_ID = ' ' OR B.JOURNAL_ID IS NULL THEN 'Y' ELSE 'N' END AS VARCHAR(255)) AS MIGRADO
	, CAST(P.SUBCATEGORIA AS VARCHAR(255)) AS SUBCATEGORIA
    , CAST(CASE
    ...
    , CAST(CASE
    	WHEN (Q.SEMESTRE ||' ('|| Q.NIVEL ||')') IS NOT NULL AND (P.SUBCATEGORIA = 'MATRICULA / CUOTA INICIAL' OR SUBSTR(P.SUBCATEGORIA,1,5) = 'CUOTA')
    		THEN (Q.SEMESTRE ||' ('|| Q.NIVEL ||')') ELSE 'OTROS INGRESOS' END AS VARCHAR(255)) AS PERIODO
    ...
    , CAST(CASE
    	...
    	ELSE 'Y' END AS VARCHAR(255)) AS ACTIVA
...
FROM ODSMGR.PS_ITEM A
		...
		INNER JOIN ODSMGR.PS_ITEM_DST B ON A.BUSINESS_UNIT = B.BUSINESS_UNIT AND A.ITEM = B.ITEM AND A.ITEM_LINE = B.ITEM_LINE AND B.LEDGER = 'ACTUALS'
											...
											AND B.JOURNAL_DATE BETWEEN TO_DATE('2020-01-01','YYYY-MM-DD') AND TO_DATE('2020-06-30','YYYY-MM-DD')
		...
		INNER JOIN ODSMGR.PS_ITEM_ACTIVITY C ON A.BUSINESS_UNIT = C.BUSINESS_UNIT AND A.ITEM = C.ITEM AND A.ITEM_LINE = C.ITEM_LINE AND B.ITEM_SEQ_NUM = C.ITEM_SEQ_NUM AND C.ENTRY_TYPE = 'PY'											
											AND C.ACCOUNTING_DT BETWEEN TO_DATE('2020-01-01','YYYY-MM-DD') AND TO_DATE('2020-06-30','YYYY-MM-DD')
