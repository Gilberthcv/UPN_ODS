SELECT BN.ACADEMIC_PERIOD_ADMITTED, BN.CAMPUS_DESC, BN.PROGRAM_DESC, BN.PERSON_UID, BN.ID, BN.NAME, BN.CUOTA_INICIAL_CONTADO, BN.TBBEXPT_DESC
    , BN.TIPO_ESTUDIANTE, CASE WHEN BN.COURSE_REFERENCE_NUMBER IS NOT NULL THEN 'SI' ELSE 'NO' END AS REGISTRO_CURSO
...
FROM (  SELECT DISTINCT A.PERSON_UID, A.ID, A.NAME, A.ACADEMIC_PERIOD_ADMITTED, A.PROGRAM, A.PROGRAM_DESC, A.CAMPUS, A.CAMPUS_DESC, A.ENROLLMENT_STATUS
        ...
        WHERE A.ACADEMIC_PERIOD_ADMITTED = B.ACADEMIC_PERIOD(+) AND A.PROGRAM = B.PROGRAM(+)
            ...
            AND A.ACADEMIC_PERIOD_ADMITTED IN ('220714','220715')) BN
    LEFT JOIN (
        ...
        WHERE C.BUSINESS_UNIT = D.BUSINESS_UNIT AND C.INVOICE = D.ITEM
            ...
            AND C.BILL_TO_CUST_ID = SPRIDEN_ID AND SPRIDEN_CHANGE_IND IS NULL) PS ON
                BN.PERSON_UID = PS.SPRIDEN_PIDM AND BN.ACADEMIC_PERIOD_ADMITTED = PS.PO_REF
