...
SELECT A.PERSON_UID, A.PROGRAM, A.TERM_CODE_EFF, A.ACADEMIC_PERIOD, A.SUBJECT, A.COURSE_NUMBER, COALESCE(A.ORIGEN,'PROYECCION') AS ORIGEN, COUNT(DISTINCT B.SHRTCKN_TERM_CODE) AS COURSE_COUNT
FROM AVANCE_CURRICULAR A, 
	( SELECT SHRTCKN_PIDM, SHRTCKN_TERM_CODE, FECHA_INICIO_NRO, SHRTCKN_SEQ_NO, SHRTCKN_SUBJ_CODE, SHRTCKN_CRSE_NUMB, SHRTCKG_SEQ_NO, SHRTCKG_GRDE_CODE_FINAL, SCREQIV_SUBJ_CODE, SCREQIV_CRSE_NUMB
		FROM ODSMGR.SHRTCKN
...
WHERE A.PERSON_UID = B.SHRTCKN_PIDM AND A.FECHA_INICIO_NRO > B.FECHA_INICIO_NRO --AND A.MIN_PERIODO <= B.FECHA_INICIO_NRO
	AND ( (A.SUBJECT = B.SHRTCKN_SUBJ_CODE AND A.COURSE_NUMBER = B.SHRTCKN_CRSE_NUMB) OR (A.SUBJECT = B.SCREQIV_SUBJ_CODE AND A.COURSE_NUMBER = B.SCREQIV_CRSE_NUMB) ) AND A.PERSON_UID = 161101
GROUP BY A.PERSON_UID, A.PROGRAM, A.TERM_CODE_EFF, A.ACADEMIC_PERIOD, A.SUBJECT, A.COURSE_NUMBER, COALESCE(A.ORIGEN,'PROYECCION')
;
