...
SELECT DISTINCT
    ...
    , CASE WHEN C.BILL_TYPE_ID IN ('F1','B1','D1','D2','C1','C2') 
        THEN (CASE WHEN G.ITEM IS NOT NULL 
                THEN G.ENTRY_AMT ELSE D.MONETARY_AMOUNT *-1 END)
        ELSE D.MONETARY_AMOUNT END AS MONTO_BI_AR_CALCULADO
...
FROM (SELECT DISTINCT 
    ...
    , PS_BI_ACCT_ENTRY D
    , (SELECT E.BUSINESS_UNIT, E.ITEM, E.ITEM_LINE, E.ITEM_SEQ_NUM, E.ENTRY_TYPE, E.ENTRY_REASON, E.ENTRY_AMT, E.ACCOUNTING_DT, F.JOURNAL_ID, F.JOURNAL_DATE
...
WHERE C.BUSINESS_UNIT = D.BUSINESS_UNIT(+) AND C.INVOICE = D.INVOICE(+)
    ...
    AND (N.LINE_SEQ_NUM = (SELECT MIN(N1.LINE_SEQ_NUM) FROM PS_BI_LINE_NOTE N1
                            WHERE N.BUSINESS_UNIT = N1.BUSINESS_UNIT AND N.INVOICE = N1.INVOICE
                                AND N1.TEXT254 LIKE '%ORIGINAL_INVOICE:%'
                            GROUP BY N1.INVOICE)
            OR N.LINE_SEQ_NUM IS NULL)
