select 'EXTERNAL_COURSE_KEY|COURSE_ID|COURSE_NAME|AVAILABLE_IND|ROW_STATUS|DURATION|START_DATE|END_DATE|TERM_KEY|DATA_SOURCE_KEY|PRIMARY_EXTERNAL_NODE_KEY|EXTERNAL_ASSOCIATION_KEY' from dual;
    SELECT DISTINCT
        ...
        'R|'||
        to_char(a.ssbsect_ptrm_start_date,'YYYYMMDD') ||'|'||
        to_char(a.ssbsect_ptrm_end_date,'YYYYMMDD') ||'|'||
        a.ssbsect_term_code ||'|'||
        ...
        CASE WHEN a.ssbsect_camp_code IN ('TML','TSI') THEN 'TRU' ELSE a.ssbsect_camp_code END ||'.'|| c.ssrattr_attr_code ||'|'||
        c.ssrattr_attr_code ||'.'|| a.ssbsect_crn
    FROM ssbsect a,
    ...
    WHERE a.ssbsect_crse_numb = b.scbcrse_crse_numb AND a.ssbsect_subj_code = b.scbcrse_subj_code
        ...
        AND a.ssbsect_ptrm_start_date <= SYSDATE +7 AND a.ssbsect_ptrm_end_date >= SYSDATE -16
