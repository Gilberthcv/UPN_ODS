...
FROM 
    ...
    GROUP BY 
        ...
        LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.RECEIVABLE_ACCOUNT_DETAIL e ON 
            ...
            SUBSTR(e.DETAIL_CODE, 1, 2) IN ('T1','TA','X1','XA','Y1','YA','Z1','ZA','TB','E1','E2') AND
            (SUBSTR(e.CROSSREF_DETAIL_CODE, 1, 2) IN ('T1','TA','X1','XA','Y1','YA','Z1','ZA','TB') OR e.CROSSREF_DETAIL_CODE is null) )
        LEFT JOIN (SELECT DETAIL_CODE, PROGRAM
                    FROM (SELECT DETAIL_CODE, PROGRAM, COUNT, MAX(COUNT) OVER(PARTITION BY PROGRAM) MAX_COUNT
                          FROM (SELECT SUBSTR(DETAIL_CODE,3,2) AS DETAIL_CODE, PROGRAM, COUNT(DETAIL_CODE) AS COUNT
                                FROM UPN_RPT_DM_PROD.ODSMGR.LOE_REGISTRATION_FEES
                                GROUP BY SUBSTR(DETAIL_CODE,3,2), PROGRAM))
                    WHERE COUNT = MAX_COUNT) f ON 
            COALESCE( CASE
                    ...
                    CASE
                        ...
                        ELSE NULL
                    END) = f.DETAIL_CODE AND 
            (c.PROGRAM = f.PROGRAM OR f.PROGRAM IS NULL ) )
        ...
        LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_REGISTRATION_FEES i ON
           b.ACADEMIC_PERIOD = i.REGISTRATION_TERM AND
            ...
            i.TO_CRED_HRS = 22.999 AND
            i.VISA_TYPE_CODE IS NULL
WHERE 
c.STUDENT_LEVEL = 'UG'
AND EXISTS (SELECT 1 
