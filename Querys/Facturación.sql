...
SELECT/*
    ...
    , Z.BILL_TYPE_ID, Z.INSTALL_NBR, Z.INVOICE, Z.INVOICE2, Z.LINE_SEQ_NUM, Z.DESCR, Z.NET_EXTENDED_AMT, Z.INVOICE_AMOUNT, Z.INVOICE_DT, Z.FECHA_VENCIMIENTO
    , Z.VAT_RGSTRN_ID, Z.ORIGINAL_INVOICE, Z.ORIGINAL_INVOICE2, Z.ENTRY_TYPE_BI, Z.ACCOUNT_BI, Z.JOURNAL_ID_BI, Z.JOURNAL_DATE_BI, Z.REV_RECOG_BASIS_DESC
...
SELECT DISTINCT
    D.BUSINESS_UNIT, D.INVOICE, D.BILL_STATUS, D.BILL_TYPE_ID, D.INVOICE_AMOUNT, D.INVOICE_DT, D.PO_REF, D.TOT_VAT, D.DUE_DT, D.PRODUCT, D.OPERATING_UNIT
    , D.NET_EXTENDED_AMT, D.ENTRY_TYPE ENTRY_TYPE_BI, D.BILL_TO_CUST_ID, REPLACE(CONCAT(CONCAT(I.NAME2,' '),I.NAME1),'/',' ') AS NOMBRE_COMPLETO
    , I.SUBCUST_QUAL1, I.CORPORATE_CUST_ID, REPLACE(CONCAT(CONCAT(J.NAME2,' '),J.NAME1),'/',' ') AS NOMBRE_COMPLETO_CORPORATE
    , K.INVOICE2, L.VAT_RGSTRN_ID, M.INSTALL_NBR, NVL(M.INVOICE,D.INVOICE) AS INVOICE_PLANTILLA
    ...
    /*, CASE WHEN D.BILL_TYPE_ID IN ('F1','B1','D1','D2','C1','C2') 
        THEN (CASE WHEN H.ENTRY_TYPE IN ('WO','WOC') AND H.ENTRY_REASON = 'A-DIR' 
                THEN E.MONETARY_AMOUNT ELSE E.MONETARY_AMOUNT *-1 END)
        ELSE E.MONETARY_AMOUNT END AS MONTO_BI_CALCULADO*/
    , CASE WHEN D.BILL_TYPE_ID IN ('F1','B1','D1','D2','C1','C2') 
    ...
    , D.LINE_SEQ_NUM, D.DESCR, D.REV_RECOG_BASIS
...
FROM ( SELECT DISTINCT 
        ...
        WHERE A.BUSINESS_UNIT = B.BUSINESS_UNIT AND A.INVOICE = B.INVOICE
            ...
            /*AND B.LINE_SEQ_NUM = (SELECT MIN(B1.LINE_SEQ_NUM) FROM ODSMGR.PS_BI_LINE B1
                            		WHERE B1.BUSINESS_UNIT = B.BUSINESS_UNIT AND B1.INVOICE = B.INVOICE)*/
        GROUP BY A.BUSINESS_UNIT, A.INVOICE, A.BILL_STATUS, A.BILL_TYPE_ID, A.INVOICE_AMOUNT, A.INVOICE_DT, A.PO_REF, A.TOT_VAT, A.DUE_DT
			...
			LEFT JOIN ( SELECT DISTINCT F.BUSINESS_UNIT, F.ITEM, F.ITEM_LINE, F.ITEM_SEQ_NUM, F.ENTRY_TYPE, F.ENTRY_REASON, F.ENTRY_AMT, F.ACCOUNTING_DT, G.JOURNAL_ID, G.JOURNAL_DATE
				        FROM (SELECT BUSINESS_UNIT, ITEM, ITEM_LINE, ITEM_SEQ_NUM, ENTRY_TYPE, ENTRY_REASON, ENTRY_AMT, ACCOUNTING_DT
				                ...
				                FROM ODSMGR.PS_ITEM_ACTIVITY WHERE BUSINESS_UNIT = 'PER03' AND ENTRY_TYPE NOT IN ('PY','MT') AND ACCOUNTING_DT BETWEEN TO_DATE(:FECHA_INICIO,'YYYY-MM-DD') AND TO_DATE(:FECHA_FIN,'YYYY-MM-DD')) F
			...
			LEFT JOIN ODSMGR.PS_LI_GBL_BI_INV O ON SUBSTR(N.TEXT254,LENGTH(N.TEXT254)-9) = O.INVOICE
			...
			LEFT JOIN ODSMGR.PS_ITEM R ON D.BUSINESS_UNIT = R.BUSINESS_UNIT AND D.INVOICE = R.ITEM
