--AC039
SELECT X.PERSON_UID, X.SPRIDEN_ID, X.NOMBRE_ESTUDIANTE, X.ACADEMIC_PERIOD, X.PROGRAM, X.PROGRAM_DESC, X.CAMPUS_DESC, X.FECHA_REGISTRO_CURSOS, X.TIPO_ESTUDIANTE, X.CREDITOS, X.CURSOS, X.TIPO_ESTUDIANTE_V2
FROM (
	SELECT DISTINCT
	...
	FROM ODSMGR.ACADEMIC_STUDY A
			INNER JOIN ODSMGR.STUDENT_COURSE B ON A.PERSON_UID = B.PERSON_UID AND A.ACADEMIC_PERIOD = B.ACADEMIC_PERIOD AND B.TRANSFER_COURSE_IND = 'N' AND B.REGISTRATION_STATUS IN ('RE','RW','RA','WC','RF')
