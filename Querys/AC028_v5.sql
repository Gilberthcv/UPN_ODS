...
FROM
    ...
    FROM
        ...
        FROM 
            ((GPA_BY_TERM a
                ...
                LEFT JOIN (SELECT DISTINCT
                              a.PERSON_UID, a.ID, a.NAME, a.STUDENT_LEVEL, a.CAMPUS, a.CAMPUS_DESC, a.PROGRAM, a.PROGRAM_DESC, a.ACADEMIC_PERIOD
                          FROM (SELECT PERSON_UID, ID, NAME, STUDENT_LEVEL, CAMPUS, CAMPUS_DESC, PROGRAM, PROGRAM_DESC, ACADEMIC_PERIOD
                                    , MAX(ACADEMIC_PERIOD) OVER(PARTITION BY PERSON_UID) AS MAX_PERIODO
                                FROM FIELD_OF_STUDY
                                WHERE SOURCE = 'LEARNER' AND CURRICULUM_ACTIVE_IND = 'Y'
                                    AND ACADEMIC_PERIOD <= '218512' --PERIODO
                                    AND PROGRAM LIKE '%'||CASE
                                        WHEN SUBSTR('218512',4,1) = '4' THEN '-UG' --PERIODO
                                        WHEN SUBSTR('218512',4,1) = '5' THEN '-WA' END) a --PERIODO
                          WHERE a.ACADEMIC_PERIOD = a.MAX_PERIODO) d ON
        ...
        WHERE a.GPA_TYPE = 'I' AND c.CREDITOS_APROBADOS >= 20 AND a.GPA > 0 AND a.ACADEMIC_STUDY_VALUE = 'UG' AND a.ACADEMIC_PERIOD = '218512' --PERIODO
