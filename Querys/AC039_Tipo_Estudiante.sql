...
SELECT --X.PERSON_UID, X.SPRIDEN_ID, X.NOMBRE_ESTUDIANTE, X.ACADEMIC_PERIOD, X.PROGRAM, X.PROGRAM_DESC, X.CAMPUS_DESC, X.FECHA_REGISTRO_CURSOS, X.TIPO_ESTUDIANTE AS TIPO_ESTUDIANTE_BANNER, X.CREDITOS, X.CURSOS
	...
	X.TIPO_ESTUDIANTE AS TIPO_ESTUDIANTE_BANNER, X.TIPO_ESTUDIANTE_V2 AS TIPO_ESTUDIANTE_CALCULADO
	, CASE WHEN Y.ACADEMIC_PERIOD_START IN ('220435','220534') THEN 'COHORT SEMESTRE ACTUAL' ELSE 'COHORT SEMESTRE ANTERIOR' END AS COHORT
	, COUNT(DISTINCT X.PERSON_UID) AS ESTUDIANTES
FROM (
	SELECT DISTINCT
...
WHERE X.MATERIA_XSER IS NULL
	...
	--AND X.TIPO_ESTUDIANTE = 'Continuo' AND X.TIPO_ESTUDIANTE_V2 = 'Nuevo'
GROUP BY X.TIPO_ESTUDIANTE, X.TIPO_ESTUDIANTE_V2
	, CASE WHEN Y.ACADEMIC_PERIOD_START IN ('220435','220534') THEN 'COHORT SEMESTRE ACTUAL' ELSE 'COHORT SEMESTRE ANTERIOR' END
ORDER BY X.TIPO_ESTUDIANTE_V2, X.TIPO_ESTUDIANTE DESC
;