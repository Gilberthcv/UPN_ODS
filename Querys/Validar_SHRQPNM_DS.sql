--Validar SHRQPNM - Datos Suplementarios
SELECT A.DECODE_PIDM, SPRIDEN_ID, SPRIDEN_FIRST_NAME ||' '|| REPLACE(SPRIDEN_LAST_NAME,'/',' ') AS NOMBRE_ESTUDIANTE, A.DECODE_SEQ, B.QUALIFYING_PAPER_TYPE_CODE, B.ACCEPTANCE_DATE AS FECHA_CONVOCATORIA
	, B.FIRST_TITLE_LINE, B.SECOND_TITLE_LINE, A.GORSDAV_PK_PARENTTAB, A.GORSDAV_ACTIVITY_DATE AS FECHA_CARGA, A.GORSDAV_USER_ID
...
FROM ODSMGR.LOE_SUPPLEMENTAL_ATTRI_VALUES A
		INNER JOIN ODSMGR.LOE_SPRIDEN ON A.DECODE_PIDM = SPRIDEN_PIDM||'' AND SPRIDEN_CHANGE_IND IS NULL
...
WHERE A.GORSDAV_ATTR_NAME IN ('TESIS','URL_TESIS','ACTO_TIP','MODALIDAD','RESO_NUM','RESO_FEC','DIPL_NUM','DIPL_FEC','DIPL_TIP_EMI','REG_LIBRO','REG_FOLIO','REG_REGISTRO')
	...
	--AND B.ACCEPTANCE_DATE BETWEEN TO_DATE('03/06/2020','DD/MM/YYYY') AND TO_DATE('20/06/2020','DD/MM/YYYY') --FECHA_CONVOCATORIA
GROUP BY A.DECODE_PIDM, SPRIDEN_ID, SPRIDEN_FIRST_NAME ||' '|| REPLACE(SPRIDEN_LAST_NAME,'/',' '), A.DECODE_SEQ, B.QUALIFYING_PAPER_TYPE_CODE, B.ACCEPTANCE_DATE, B.FIRST_TITLE_LINE
	, B.SECOND_TITLE_LINE, A.GORSDAV_PK_PARENTTAB, A.GORSDAV_ACTIVITY_DATE, A.GORSDAV_USER_ID
;