...
FROM (  SELECT 
        ...
        FROM ODSMGR.HOLD a
        		INNER JOIN ODSMGR.PS_BI_HDR b ON a.ID = b.BILL_TO_CUST_ID AND SUBSTR(a.HOLD_EXPLANATION,1,6) = b.PO_REF
        		INNER JOIN ODSMGR.PS_BI_LINE c ON b.INVOICE = c.INVOICE
        											AND (SUBSTR(c.DESCR,1,2) = 'FM' OR UPPER(c.DESCR) LIKE '%MATR%' OR UPPER(c.DESCR) LIKE '%PRONABEC%MAT%'
        												OR UPPER(c.DESCR) LIKE '%ARANCEL CONTADO%'
														OR SUBSTR(c.DESCR,1,2) = 'T1' OR UPPER(c.DESCR) LIKE '%CUOTA%INI%' OR UPPER(c.DESCR) LIKE '%CUOTA 0%' OR UPPER(c.DESCR) LIKE '%CUOTA 1%')
        		LEFT JOIN ODSMGR.PS_ITEM D ON b.INVOICE = d.ITEM
        WHERE a.HOLD IN ('M1','F1','C1','C2') AND a.ACTIVE_HOLD_IND = 'Y' AND SUBSTR(a.HOLD_EXPLANATION,1,3) IN ('220','221')
        	AND CASE a.HOLD WHEN 'M1' THEN 'FM' WHEN 'F1' THEN 'TF' WHEN 'C1' THEN 'T1' WHEN 'C2' THEN 'T1' END = 
        		CASE WHEN SUBSTR(c.DESCR,1,2) = 'FM' OR UPPER(c.DESCR) LIKE '%MATR%' OR UPPER(c.DESCR) LIKE '%PRONABEC%MAT%' THEN 'FM'
        			WHEN UPPER(c.DESCR) LIKE '%ARANCEL CONTADO%' THEN 'TF'
        			WHEN SUBSTR(c.DESCR,1,2) = 'T1' OR UPPER(c.DESCR) LIKE '%CUOTA%INI%' OR UPPER(c.DESCR) LIKE '%CUOTA 0%' OR UPPER(c.DESCR) LIKE '%CUOTA 1%' THEN 'T1'
        			END
		) x
     , (SELECT DISTINCT
