...
FROM 
    ...
    FROM 
        ...
        FROM
            UPN_RPT_DM_PROD.ODSMGR.STUDENT_COURSE 
        WHERE 
    ...
    GROUP BY 
        ...
        TOTAL_CREDITOS ) b
        INNER JOIN UPN_RPT_DM_PROD.ODSMGR.ACADEMIC_STUDY c ON
            b.PERSON_UID = c.PERSON_UID AND
            b.ACADEMIC_PERIOD = c.ACADEMIC_PERIOD )
        LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_SPRIDEN d ON 
            b.PERSON_UID = d.SPRIDEN_PIDM AND
            d.SPRIDEN_CHANGE_IND IS NULL )
        LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.RECEIVABLE_ACCOUNT_DETAIL e ON 
        ...
        LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_BASE_TABLE_EQUIV f ON 
            COALESCE( CASE
        ...
        LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_BASE_TABLE_EQUIV g ON 
            CASE
                WHEN SUBSTR(e.DETAIL_CODE, 1, 1) = 'E'
                THEN SUBSTR(e.DETAIL_CODE, 3, 2)
                ELSE NULL
        ...
        LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_REGISTRATION_FEES h ON
        ...
        LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_REGISTRATION_FEES i ON
...
AND EXISTS (SELECT 1 
          ...
          WHERE TRM.academic_period = b.ACADEMIC_PERIOD
